...
**Version:** 2.4.33 · **Updated:** 2026-09-14  
**Release:** https://github.com/ShadowESC95/ELI_v2.0/releases/tag/v2.4.33
...
|----------|----------|--------------|
| Windows, simplest | `ELI-Setup-2.4.33.exe` | Run Setup |
...
### Linux AppImage
```bash
chmod +x ELI_v2-2.4.33-x86_64.AppImage
./ELI_v2-2.4.33-x86_64.AppImage
```

### Linux portable
```bash
tar -xzf ELI_v2-2.4.33-linux-portable.tar.gz
cd ELI_v2-2.4.33-linux-portable
chmod +x ELI_Setup.sh && ./ELI_Setup.sh
./RUN_ELI.sh
```

### Windows Setup
Run **`ELI-Setup-2.4.33.exe`**. Desktop shortcut is **ELI only** (Server in Start Menu). A second GUI instance is refused (singleton). Data: `%LOCALAPPDATA%\ELI_v2\`.
...
| Menu / Server icon opens deleted AppImage | 2.4.33+ auto-refreshes `.desktop` on version bump; or `./ELI_v2-*-x86_64.AppImage --integrate` |
| “Yes” to install mpv does nothing / LLM talks | 2.4.33+ Phase-45 `CONFIRM_PENDING_REMEDIATION` (first yes runs apt) |
...
| “just checking in / what have you been doing?” → version/git data dump | 2.4.33+ “checking in” is rapport, not maintenance SELF_REPORT; keeps selected reasoning mode |
...
| Big GGUF “loaded” then aborts on first decode (ctx/batch ≫ VRAM) | 2.4.33+ load probe runs when **ctx or batch** exceed smart-fit, not only GPU layers |
