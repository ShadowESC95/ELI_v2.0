...
def main():
    ...
    ap.add_argument("--allow-haar-fallback", action="store_true", help="Diagnostic only. Not recommended for desktop gaze control.")
    ap.add_argument("--preview", action="store_true", help="Open tracker verifier before calibration starts.")
    args = ap.parse_args()
    ...
    if extractor.method != "mediapipe_facemesh_iris":
        print("[!] MediaPipe iris tracking is not active; desktop gaze calibration will not work.")
        if getattr(extractor, "backend_error", ""):
            print(f"[!] MediaPipe import/init error: {extractor.backend_error}")
        print("[!] Fix: python -m pip install -r requirements_optional_mediapipe.txt")
        print("[!] Then verify: python scripts/eli_gaze_verify_tracker.py --camera auto")
        if not args.allow_haar_fallback:
            cap.release()
            raise SystemExit(2)
        print("[!] Continuing with Haar fallback only because --allow-haar-fallback was passed.")
    ...
    if args.preview:
        print("[+] Preview requested. Run the standalone verifier if you need a longer test: python scripts/eli_gaze_verify_tracker.py --camera auto")
        ...
        for pid, (tx, ty) in enumerate(pts):
            ...
            while accepted < args.samples_per_point and attempts < args.samples_per_point * 4:
                ...
                if not ok:
                    continue
                sample = extractor.extract(frame)
                min_conf = 0.50 if extractor.method == "mediapipe_facemesh_iris" else 0.35
                if sample is not None and sample.confidence >= min_conf:
                    samples.append({
                        "point_id": pid,
                        "target_x": float(tx),
                        "target_y": float(ty),
    ...
    if len(samples) < 30:
        raise SystemExit("[!] No usable calibration samples were collected. Verify tracker first: python scripts/eli_gaze_verify_tracker.py --camera auto")
