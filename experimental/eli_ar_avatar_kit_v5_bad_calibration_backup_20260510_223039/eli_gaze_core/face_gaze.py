...
class FaceGazeExtractor:
    ...
    def _extract_mediapipe(self, frame_bgr: np.ndarray) -> Optional[GazeSample]:
        ...
        left_iris = centre(left_iris_ids) if len(pts) > 472 else centre(left_eye_ids)
        right_iris = centre(right_iris_ids) if len(pts) > 477 else centre(right_eye_ids)
        ...
        confidence = 0.85
        if not (0.15 <= le_rx <= 0.85 and 0.15 <= re_rx <= 0.85):
            confidence *= 0.75
