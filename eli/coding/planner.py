...
_IMPLEMENT_RULES = (
    ...
    "- Implement the REAL solution the task actually requires: the genuine algorithm,\n"
    "  computation, or domain logic — correct and complete. This is production code, not a demo.\n"
    "- FORBIDDEN (these are toys, and a toy is a FAILURE): hardcoded or faked results; dummy\n"
    "  heuristics that only pretend to do the work; trivial stand-ins for the real algorithm;\n"
    "  stubs, empty bodies, bare `pass`; 'simplified for demonstration' shortcuts; or any logic\n"
    "  that misrepresents the problem instead of solving it. If the task names a known problem\n"
    "  or algorithm (sorting, parsing, search, graph, SAT, parsing, crypto, etc.), implement it\n"
    "  CORRECTLY — do not approximate it with something that merely looks plausible.\n"
    "- The program must genuinely DO what the task asks and produce real, correct output for real\n"
    "  inputs — not print a plausible-looking constant. Handle real inputs and edge cases.\n"
