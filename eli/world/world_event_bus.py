...
def fire_memory_uncertainty_event(reason: str, memory_id: Any = None) -> None:
    ...
    vector) — not simply "no vector store configured" (a permanent, expected
    condition on some installs, not an uncertainty signal). Drives the World
    tab's memory_uncertainty awareness bar, which previously had no real code
    path firing it (only a manual "Mark Memory Fog" test button)."""
    fire_world_event(
        "memory_uncertainty",
        "memory",
