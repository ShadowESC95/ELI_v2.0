...
try:
    ...
    ELI_ROOT = canonical_root(Path(__file__).resolve().parents[3])
except Exception:
...
_add("Conversation & persona", "PERSONA_REFRESH", "Refresh/reload ELI's persona from the latest profile", "“refresh your persona”", "“reload your persona”")
...
_add("App & window control", "MINIMISE_WINDOW", "Minimise the focused window", "“minimise the window”", "“minimise this”")
_add("App & window control", "MINIMIZE_WINDOW", "Minimise the focused window (US spelling)", "“minimize this window”")
_add("App & window control", "MINIMISE_APP", "Minimise a specific app's window", "“minimise spotify”")
_add("App & window control", "MINIMIZE_APP", "Minimise a specific app's window (US spelling)", "“minimize chrome”")
_add("App & window control", "HIDE_APP", "Hide an application's window", "“hide spotify”", "“hide the window”")
...
_add("System status & info", "GET_WEATHER", "Weather for a place", "“weather in Paris”")
...
_add("Grounded introspection", "EXPLAIN_GGUF_DIAGNOSTICS", "Live GGUF load/inference compatibility report (runtime version, preflight, last load error)", "“gguf errors”", "“why won't nemotron load”", "“empty response from the model”")
_add("Grounded introspection", "EXPLAIN_FAILURE_LOG", "Grounded executor failure log from agent.sqlite3 (not GGUF console lines)", "“what are the failures”", "“show the failure log”", "“what's tripping you up”")
_add("Grounded introspection", "EXPLAIN_LAST_FAILURE", "Explain the most recent grounded or executor failure", "“what was the last failure”", "“why did that fail”")
...
_add("Self-maintenance", "AUTOPILOT_DEBUG", "Turn a failure into a plan: root cause, affected files from git history, rollback, patch and validation", "“debug this error: ValueError x in eli/core/paths.py”", "“autopilot debug this traceback”")
_add("Self-maintenance", "CODEBASE_GRAPH", "Import graph of ELI's own source: what depends on what", "“show the codebase graph”")
_add("Self-maintenance", "TIMESTAMP_DIAG", "Run real clock and timestamp diagnostics instead of guessing", "“check the timestamps”", "“investigate the timestamp problem”")
_add("Voice", "DESIGN_VOICE", "Build a custom style voice from a base voice with pitch, speed and effects", "“design a voice called Nova, deep and slow”", "“make a custom voice named Nova”")
_add("Conversation & persona", "SET_COMMUNICATION_STYLE", "Persist how you want ELI to speak to you (empty or a reset phrase clears it)", "“speak concisely from now on”")
_add("Conversation & persona", "SET_AI_MODE", "Set the reasoning mode (Quick, Normal, Advanced, Research, Expert); reached through the intent resolver or the mode selector", "(no dedicated phrase)")
_add("Memory & profile", "USER_INFO_REPORT", "Full report of the living user profile; reached through the intent resolver", "(no dedicated phrase)")
_add("Vision & screen", "IMAGE_STATUS", "Status of the latest image job", "“what is the latest image status”")
_add("Generation (docs/code)", "DOC_GENERATE", "Alias of GENERATE_DOCUMENT", "“generate a document about tides”")
_add("Plugins", "PLUGIN_STATUS", "Status of one plugin; reached through the intent resolver (“list plugins” lists them all)", "(no dedicated phrase)")
_add("Plugins", "MCP_LIST", "List configured MCP servers", "“list mcp servers”")
_add("Plugins", "MCP_STATUS", "Status of MCP servers and their tools", "“mcp status”")
_add("Plugins", "MCP_TOOLS", "List the tools MCP servers expose", "“list mcp tools”")
_add("Plugins", "MCP_CALL", "Call a tool on an MCP server (the server runs outside ELI's network guard)", "“call the mcp tool read_file”")
_add("Plugins", "MCP_ADD", "Add an MCP server (installed switched off until you enable it)", "“add an mcp server”")
_add("Plugins", "MCP_REMOVE", "Remove an MCP server", "“remove the mcp server github”")
_add("Plugins", "MCP_DOCTOR", "Check MCP servers: launch preflight and handshake", "“mcp doctor”", "“check my mcp servers”")
...
_add("Voice", "WAKE_SET", "Set your own wake word (any phrase); ELI persists it and trains the detector on it", "“change the wake word to athena”", "“set my wake word to atlas”")
...
def generate_capabilities_doc() -> Dict:
    ...
    out.append(f"*Auto-generated {datetime.date.today().isoformat()} from "
               ...
               f"{len(routable)} routable or executor-backed). Regenerated by "
