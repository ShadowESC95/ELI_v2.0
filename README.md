...
> **v2.4.47 — the routable-action audit is complete: all 186 actions individually checked against their handler's actual return shape.** Third and final pass, following v2.4.44/v2.4.45: added `TRANSCRIBE` and `OCR_IMAGE` (raw transcribed/recognized text — same "must never be paraphrased" class as `READ_FILE`/`SHELL_EXEC`/`MCP_CALL`) plus 19 more confirmation-style actions (`HELP`, `LIST_CAPABILITIES`, `MEMORY_RECALL`, `SMART_HOME`, `CREATE_DOCUMENT`, `DESIGN_VOICE`/`CREATE_VOICE`, `PROACTIVE_START`/`STOP`, etc.) to the quick-mode verbatim set. Two actions were individually checked and deliberately left as LLM-synthesized: `FIX_FILE` (its `content` is a machine-readable JSON event for the GUI, not prose — showing it verbatim would put raw JSON in the chat) and `RUN_TESTS` (its own code comment says the design intent is "summarise it in chat", not a raw dump). The remaining ~17 unaudited actions (`CHAT`, `WEB_SEARCH`, `GENERATE_SCRIPT`, `ANALYZE_IMAGE`/`PDF`, `SEQUENCE`, `EXECUTE_GOAL`, ...) are genuinely creative or multi-step, where LLM synthesis is the correct behavior, not a gap.
>
