...
4. **Repo hygiene.** Committed junk at root: empty `...` and `[package-index-options]`
   ...
   is.
5. **Tests not green.** ~103 test files exist (good instinct) but the suite has
   ~24 pre-existing failures (missing modules, outdated routing assertions). A
   suite you don't keep green stops being a safety net.
