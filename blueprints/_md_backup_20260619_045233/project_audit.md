...
| Metric | Value |
|---|---|
| Python files under `eli/` | **360** |
| Python LOC under `eli/` | **138,243** |
...
| Capabilities (`capability_manifest.json`, generated 2026-06-17) | **208** |
| `SUPPORTED_ACTIONS` (`executor_enhanced.py:1899`) | **174** |
| Specialist bus agents | **14** (named in §4.4) |
| Test files / test LOC | **160 / 18,355**; collain **6,864 tests** |
...
### 4.1 Router — `execution/router_enhanced.py` (7,076)
...
### 4.2 Executor — `execution/executor_enhanced.py` (14,294)
...
### 4.3 Kernel / engine — `kernel/engine.py` (13,326)
...
### 4.4 Agent bus — `cognition/agent_bus.py` (3,073)
...
### 4.5 Inference — `cognition/gguf_inference.py` (2,538) + `inference_broker.py`
...
### 4.6 Memory — `memory/memory.py` (4,415) + `memory/`
...
- **STT** `audio_stt.py` (1,848): faster-whisper `small.en`, CPU, int8 *(runtime snapshot)*. **[L]**
...
`proactive_daemon.py` (1,270): continuous habit/pattern learning, persona-overlay refresh,
...
### 4.10 Learning / self-improvement — `learning/` (12) + `runtime/self_improvement.py` (1,206)
...
The catch-all: `deterministic_grounding_gate.py` (4,292), `grounded_remediation.py` (1,603),
`generated_script_guard.py` (1,072), `control_contracts.py` (943), `security.py` (206),
...
PyQt app `eli_pro_audio_gui_MKI.py` (10,985) + `labs_tab.py` (5,562) + panels. 12 main tabs
...
- **`eli/guards/`** (1 file, 0 LOC) and **`eli/integrations/`** (1 file, 0 LOC) — empty packages. **[V]**
...
- 160 test files / 18,355 LOC; **6,864 tests collect** cleanly. **[V]**
