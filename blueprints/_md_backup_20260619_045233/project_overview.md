...
4. **Repo hygiene.** Committed junk at root: empty `...` and `[package-index-options]`
   ...
   is.
5. **Tests are GREEN (measured 2026-06-19).** 162 test files; `pytest tests/` =
   **6,880 passed / 42 skipped / 2 xfailed / 0 failed** (6,924 collected, ~7m25s on
   the `.venv`/GPU). The `tests/claims/` contract layer makes it a real safety net.
