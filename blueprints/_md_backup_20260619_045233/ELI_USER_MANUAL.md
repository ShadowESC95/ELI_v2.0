...
- Start it with the **ELI Server** desktop shortcut, or `scripts/eli_serve.sh` (Mac/Linux) /
  `scripts/eli_serve.ps1` (Windows). Full details: `docs/SERVER_AND_WEB_APP.md`.
- Over the phone you get chat, memory, search, news, documents, scheduling, status — everything
  except physically controlling *that PC's* screen/mouse/camera (those run on the host).
...
*That's the whole assistant — inside and out. You don't need any of the last two appendices to
