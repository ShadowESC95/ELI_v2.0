...
> **Updated for v2.4.69 (September 2026).** One hardware-aware GUI wizard for every supported OS.
...
- Windows: desktop shortcut is **ELI only** (Server is Start Menu). If two windows open, close one — v2.4.69+ refuses a second GUI instance.
...
```bash
./ELI_v2-2.4.69-x86_64.AppImage --integrate      # add/refresh menu entries
./ELI_v2-2.4.69-x86_64.AppImage --uninstall       # remove them
```
