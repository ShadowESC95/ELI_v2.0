...
*Auto-generated 2026-09-08 from `capability_manifest.json` (227 capabilities; 210 routable). Regenerated by `eli/tools/registry/capabilities_doc.py` alongside the manifest.*
...
| `MOUSE_CONTROL` | Move/click the mouse (relative deltas + named targets via SCREEN_LOCATE) | “move cursor right 50 pixels” · “click the submit button” | Core |

## Media

| Action | What it does | Example activation phrase(s) | Source |
|---|---|---|---|
| `PLAY_MEDIA` | Play on the named platform — Spotify/Netflix native apps; YouTube via visible mpv (or browser for youtube.com) | “play Juicy by Notorious B.I.G. on Spotify” · “play lo-fi on YouTube” | Core |
...
| Action | What it does | Example activation phrase(s) | Source |
|---|---|---|---|
| `SCREEN_READ_ANALYZE` | Screenshot → local VL + OCR (+ memory/research fusion on deep asks) | “what's on my screen” · “analyze my screen in depth” · “exactly what's on my screen” · “how does this relate to my research” · “do you remember seeing this before” | Core |
| `SCREEN_LOCATE` | Find/click on-screen UI (AT-SPI → local VL-ground → OCR) | “click the submit button” · “find play on screen” · “click the blue icon” | Core |
...
| `ANALYZE_IMAGE` | Describe/analyse an image file (local VLM) | “describe cat.jpg” · “analyse this image” | Core |
...
| `AMBIENT_VISION` | Toggle continuous local screen-watching (glances → memory) | “watch my screen” · “stop watching” | Core |
...
| `MCP_ADD` | — | (needs activation phrase) | Core |
| `MCP_CALL` | — | (needs activation phrase) | Core |
| `MCP_DOCTOR` | — | (needs activation phrase) | Core |
| `MCP_LIST` | — | (needs activation phrase) | Core |
| `MCP_REMOVE` | — | (needs activation phrase) | Core |
| `MCP_STATUS` | — | (needs activation phrase) | Core |
| `MCP_TOOLS` | — | (needs activation phrase) | Core |
...
| `TIMESTAMP_DIAG` | — | (needs activation phrase) | Core |
...
- Routable actions documented: **193/210**
- ⚠️ New/undocumented (need a curated phrase): AUTOPILOT_DEBUG, CODEBASE_GRAPH, DESIGN_VOICE, DOC_GENERATE, IMAGE_STATUS, MCP_ADD, MCP_CALL, MCP_DOCTOR, MCP_LIST, MCP_REMOVE, MCP_STATUS, MCP_TOOLS, PLUGIN_STATUS, SET_AI_MODE, SET_COMMUNICATION_STYLE, TIMESTAMP_DIAG, USER_INFO_REPORT
