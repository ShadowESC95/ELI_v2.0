...
*Generated 2026-06-07 from `capability_manifest.json` (194 capabilities) + the live executor action surface (155 routable actions).*

**How to read this:** every row is a real action ELI can route to and execute.
Activation phrases are **representative** — the router is paraphrase- and
typo-tolerant and accepts many wordings; a phrase shows the *shape* that fires
the action, not the only accepted form. `Plugin` marks capabilities backed by a
plugin (rest are core executor+router). Voice users prefix with the wake word
(“computer, …”). Generation actions (documents/scripts/projects) now run the
...
| Action | What it does | Example activation phrase(s) | Source |
|---|---|---|---|
| `SCREEN_READ_ANALYZE` | Screenshot → vision model + OCR description; answers questions about on-screen content | “what's on my screen” · “can you see this” · “what season is on the screen” | Core |
...
These appear in the manifest but are synonyms normalised to a routable action, post-actions, or internal pipeline steps:

`ANALYZE_PDF_FOLDER`, `ANSWER`, `CANCEL_PENDING_REMEDIATION`, `CHECK_TARGET_STATUS`, `CLOSE_APPLICATION`, `CONFIRM_PENDING_REMEDIATION`, `CREATE_DOC`, `CREATE_SCRIPT`, `DIRECT_RESPONSE`, `DOC_GENERATE`, `EXIT_APP`, `EXPLAIN_LAST_FAILURE`, `GENERATE_CODE`, `HIDE_APP`, `IMAGE_STATUS`, `LAUNCH_APP`, `LISTEN_FOR_COMMAND`, `MINIMISE_APP`, `MINIMISE_WINDOW`, `MINIMIZE_APP`, `MINIMIZE_WINDOW`, `NOOP`, `OPEN_APPLICATION`, `PLUGIN_STATUS`, `PREPARE_REMEDIATION`, `QUIT_APP`, `ROUTING_FAULT_EXPLAIN`, `SAY`, `SEQUENCE_STEP`, `SET_AI_MODE`, `SET_USER_NAME`, `SKIP_YOUTUBE_AD`, `STT_DIAGNOSTICS`, `TEMPLATE`, `USER_INFO_REPORT`, `VOICE_DIAGNOSTICS`, `WRITE_CODE`, `WRITE_DOCUMENT`, `WRITE_SCRIPT`


## Coverage

- Routable actions documented: **155/155**
- Note (documented but not in SUPPORTED set): ANALYZE_PDF_FOLDER

*Totals: 155 routable actions + 13 plugin-backed + ~46 aliases/internal = 194 manifest capabilities.*
