...
*Auto-generated 2026-09-26 from `capability_manifest.json` (228 capabilities; 211 routable or executor-backed). Regenerated by `eli/tools/registry/capabilities_doc.py` alongside the manifest.*
...
| `SET_COMMUNICATION_STYLE` | Persist how you want ELI to speak to you (empty or a reset phrase clears it) | “speak concisely from now on” | Core |
| `SET_AI_MODE` | Set the reasoning mode (Quick, Normal, Advanced, Research, Expert); reached through the intent resolver or the mode selector | (no dedicated phrase) | Core |
...
| `IMAGE_STATUS` | Status of the latest image job | “what is the latest image status” | Core |
...
| `DESIGN_VOICE` | Build a custom style voice from a base voice with pitch, speed and effects | “design a voice called Nova, deep and slow” · “make a custom voice named Nova” | Core |
...
| `STT_DIAGNOSTICS` | Mic/STT/TTS diagnostics (Whisper, wake word, ducking, Piper) | “run voice diagnostics” · “voice diagnostics” · “check the microphone” · “stt diagnostics” | Core |
...
| `DOC_GENERATE` | Alias of GENERATE_DOCUMENT | “generate a document about tides” | Core |
...
| `GET_WEATHER` | Weather for a place | “weather in Paris” | Plugin |
| `NEWS_FETCH` | Synthesised news digest; topic-scoped re-fetch after a briefing when you go deeper | “what's the news” · “catch me up” · “go deeper into Hubble” · “more on the transformer story” | Core |
...
| `EXPLAIN_GGUF_DIAGNOSTICS` | Live GGUF load/inference compatibility report (runtime version, preflight, last load error) | “gguf errors” · “why won't nemotron load” · “empty response from the model” | Core |
| `EXPLAIN_FAILURE_LOG` | Grounded executor failure log from agent.sqlite3 (not GGUF console lines) | “what are the failures” · “show the failure log” · “what's tripping you up” | Core |
| `EXPLAIN_LAST_FAILURE` | Explain the most recent grounded or executor failure | “what was the last failure” · “why did that fail” | Core |
...
| `MEMORY_FORGET` | Delete matching memories and everything derived from them, after confirmation | “forget that my locker is 212” | Core |
...
| `USER_INFO_REPORT` | Full report of the living user profile; reached through the intent resolver | (no dedicated phrase) | Core |
...
| `SELF_REPAIR_PLAYBOOK` | Grounded repair decision guide (self help) | “self help” · “repair playbook” · “maintenance playbook” | Core |
| `AUTOPILOT_DEBUG` | Turn a failure into a plan: root cause, affected files from git history, rollback, patch and validation | “debug this error: ValueError x in eli/core/paths.py” · “autopilot debug this traceback” | Core |
| `CODEBASE_GRAPH` | Import graph of ELI's own source: what depends on what | “show the codebase graph” | Core |
| `TIMESTAMP_DIAG` | Run real clock and timestamp diagnostics instead of guessing | “check the timestamps” · “investigate the timestamp problem” | Core |
...
| Action | What it does | Example activation phrase(s) | Source |
|---|---|---|---|
| `PLUGIN_STATUS` | Status of one plugin; reached through the intent resolver (“list plugins” lists them all) | (no dedicated phrase) | Core |
| `MCP_LIST` | List configured MCP servers | “list mcp servers” | Core |
| `MCP_STATUS` | Status of MCP servers and their tools | “mcp status” | Core |
| `MCP_TOOLS` | List the tools MCP servers expose | “list mcp tools” | Core |
| `MCP_CALL` | Call a tool on an MCP server (the server runs outside ELI's network guard) | “call the mcp tool read_file” | Core |
| `MCP_ADD` | Add an MCP server (installed switched off until you enable it) | “add an mcp server” | Core |
| `MCP_REMOVE` | Remove an MCP server | “remove the mcp server github” | Core |
| `MCP_DOCTOR` | Check MCP servers: launch preflight and handshake | “mcp doctor” · “check my mcp servers” | Core |
...
`ANSWER`, `CHECK_TARGET_STATUS`, `CLOSE_APPLICATION`, `CREATE_DOC`, `CREATE_SCRIPT`, `DIRECT_RESPONSE`, `EXIT_APP`, `GENERATE_CODE`, `LAUNCH_APP`, `OPEN_APPLICATION`, `PREPARE_REMEDIATION`, `QUIT_APP`, `SAY`, `VOICE_DIAGNOSTICS`, `WRITE_CODE`, `WRITE_DOCUMENT`, `WRITE_SCRIPT`


## Coverage

- Routable actions documented: **211/211**
