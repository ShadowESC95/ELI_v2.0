...
> **Updated for v2.3.54 (August 2026).** **AUDIO IS BACK** — cross-platform microphone
> auto-resolve (no env vars on Linux/macOS/Windows). Primary install: prebuilt installers on
> [GitHub Releases](https://github.com/ShadowESC95/ELI_v2.0/releases/tag/v2.3.54) —
> `ELI-Setup-<v>.exe` (Windows), `.dmg` (macOS Apple Silicon), `.AppImage` (Linux).


**Version:** 2.3.54  
**Audience:** First-time users on Linux, Windows, or macOS  
**Release page:** https://github.com/ShadowESC95/ELI_v2.0/releases/tag/v2.3.54
...
tar -xzf ELI_v2-2.3.54-linux-portable.tar.gz
cd ELI_v2-2.3.54-linux-portable
```
...
```bash
cd ~/Desktop/ELI_v2-2.3.54-linux-portable
...
From https://github.com/ShadowESC95/ELI_v2.0/releases/tag/v2.3.54:

- **Portable:** `ELI_v2-2.3.54-windows-x64.zip` — extract anywhere
- **Installer:** `ELI-Setup-2.3.54.exe` — graphical install (per-user, no admin; shows the licence first)
...
```bash
cd ~/Desktop
curl -LO https://github.com/ShadowESC95/ELI_v2.0/releases/download/v2.3.54/ELI_v2-2.3.54-linux-portable.tar.gz
tar -xzf ELI_v2-2.3.54-linux-portable.tar.gz
cd ELI_v2-2.3.54-linux-portable
chmod +x ELI_Setup.sh
./ELI_Setup.sh
...
```bash
cd /path/to/ELI_v2-2.3.54-linux-portable   # or your install root
...
```bash
cd /path/to/ELI_v2-2.3.54-linux-portable
./scripts/eli_serve.sh --lan --https
```
...
```bash
/home/you/Desktop/ELI_v2-2.3.54-linux-portable/scripts/eli_serve.sh --lan --https
```
...
```bash
cd ELI_v2-2.3.54-linux-portable && ./RUN_ELI.sh
```

### Linux — phone server with voice

```bash
cd ELI_v2-2.3.54-linux-portable && ./scripts/eli_serve.sh --lan --https
```
...
*ELI v2.0 — local, private, yours. This guide matches release v2.3.54.*
