...
class EliMainWindow(QMainWindow):
    ...
    def _speak_response(self, text: str):
        ...
        def _run():
            try:
                from eli.tools.io.tts_router import speak_text as _eli_gui_speak_text
                p = _eli_gui_speak_text(clean, piper_path=PIPER, model_path=MODEL)
...
'''
    ...
    def send_message(self):
        ...
        def generate_worker():
            try:
                ...
                try:
                    import re as _eli_fast_re
                    _msg_norm = " ".join(str(user_message or "").strip().lower().split())
                    _identity_or_phatic_fastpath = bool(_eli_fast_re.search(
                        r"\b(?:"
                    ...
                    _identity_or_phatic_fastpath = False

                if _identity_or_phatic_fastpath and _ce is not None:
                    print("[GUI] FASTPATH -> CognitiveEngine.process (identity/phatic)")
                    try:
                        result = _ce.process(
                            user_message,
                            stream=True,
                        ...
                        _ce_ok = True
                    except Exception as _ce_fast_err:
                        print(f"[GUI] FASTPATH CognitiveEngine.process failed: {_ce_fast_err}")
                        result = None
                        _ce_ok = False
                else:
                    try:
                ...
                if not _orchestrator_ok and not _ce_ok:
    ...
    def closeEvent(self, event):
        ...
        _boot = float(getattr(self, "_debug_boot_ts", _time.time()))
        _uptime = _time.time() - _boot
        print(f"[GUI-CLOSE] closeEvent received uptime={_uptime:.2f}s spontaneous={event.spontaneous()} accepted={event.isAccepted()}")
        _tb.print_stack(limit=40)

        if not getattr(self, "_debug_first_close_seen", False):
            self._debug_first_close_seen = True
            print("[GUI-CLOSE] Ignoring first close event for debugging.")
            event.ignore()
            return
