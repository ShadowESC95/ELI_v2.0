...
- Volume commands now use system fallback (wpctl/amixer) instead of executor.
- Fixed indentation errors that caused 'process_input_stream' AttributeError.
- Added TTS methods _init_tts and speak.
"""
...
class ELIEntropicGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self.tts_enabled = True  # Added TTS flag
    ...
    def _init_tts(self):
        try:
            ...
            # Try to set am-us voice if available
            voices = self.tts_engine.getProperty('voices')
            for voice in voices:
                if 'am-us' in voice.id.lower() or 'en_US' in voice.id:
    ...
    def speak(self, text):
        if getattr(self, 'tts_enabled', False) and self.tts_engine:
            try:
