...
from PySide6.QtWidgets import QComboBox, QToolBar, QLabel
...
class ELIEntropicGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # Initialize UI
        self.init_ui()
        self.init_toolbar()   # <-- Toolbar created here
    ...
    # =============== TTS METHODS ===============
    def _init_tts(self):
        """Initialize Piper TTS with Amy voice."""
        self.tts_engine = None
        self.tts_available = False
        # Locate Piper binary or Python package
        # We'll use the command-line piper if available, or fallback to Python package
        ...
        if self.piper_path is None:
            ...
            try:
                ...
                print("✅ Piper TTS (Python module) loaded")
            except ImportError:
                print("⚠️ Piper TTS not installed. Install with: pip install piper-tts")
                self.tts_available = False
        else:
            self.tts_available = True
            print(f"✅ Piper TTS (binary) found at {self.piper_path}")

        # Locate Amy voice model
        self.voice_path = Path.home() / "ELI_AI" / "piper_voices" / "en_US-amy-medium.onnx"
        if not self.voice_path.exists():
            print(f"⚠️ Amy voice model not found at {self.voice_path}. TTS will be disabled.")
            self.tts_available = False
    ...
    def speak(self, text):
        ...
        try:
            if self.piper_path:
                ...
                import numpy as np
                # Load voice
                voice = piper.load_voice(str(self.voice_path))
                # Synthesize
    ...
    def init_toolbar(self):
        ...
        mode_label = QLabel("Reasoning Mode: ")
        toolbar.addWidget(mode_label)

        self.mode_combo = QComboBox()
        self.mode_combo.addItems(["Quick", "Deep", "Debate", "Reflection", "Multi-Agent"])
        self.mode_combo.currentTextChanged.connect(self.change_reasoning_mode)
        ...
        self.tts_action.setChecked(self.tts_enabled)
        self.tts_action.triggered.connect(self.toggle_tts)
        toolbar.addAction(self.tts_action)

        self.ac_action = QAction("✍️ AutoCorrect", self)
        self.ac_action.setCheckable(True)
        self.ac_action.setChecked(self.autocorrect_enabled)
        self.ac_action.triggered.connect(self.toggle_autocorrect)
        toolbar.addAction(self.ac_action)

        # Force visibility
        toolbar.setVisible(True)
        toolbar.show()

    def change_reasoning_mode(self, mode):
        mode_map = {
            "Quick": "quick",
            "Deep": "deep",
            "Debate": "debate",
        ...
        new_mode = mode_map.get(mode, "deep")
        self.engine.set_reasoning_mode(new_mode)
        self.add_system_message("SYSTEM", f"Reasoning mode changed to {mode}", "#5e81ac")

    def toggle_tts(self, checked):
        self.tts_enabled = checked
        status = "enabled" if checked else "disabled"
        self.add_system_message("SYSTEM", f"Text-to-Speech {status}", "#88c0d0")

    def toggle_autocorrect(self, checked):
        self.autocorrect_enabled = checked
        status = "enabled" if checked else "disabled"
        self.add_system_message("SYSTEM", f"Autocorrect {status}", "#88c0d0")

    # =============== AUTO CORRECT ===============
    def apply_autocorrect(self, text):
