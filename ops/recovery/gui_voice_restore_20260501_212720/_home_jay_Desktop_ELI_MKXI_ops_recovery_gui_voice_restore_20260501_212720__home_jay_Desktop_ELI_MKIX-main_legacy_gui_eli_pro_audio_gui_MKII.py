...
class _GUIEngineAdapter:
    ...
    def parse_intent(self, user_input: str, context: list) -> dict:
        try:
            import eli.tools.automation.executor_bridge as _eb
            return _eb.route_command(user_input)
        except Exception:
    ...
    def assemble_precise_context(self, user_input: str, working_memory,
        ...
        _bus_ctx = self._dispatch_agent_bus(user_input, intent)
    ...
    def enqueue_post_response_storage(self, user_input: str, response: str,
        ...
        try:
            ...
            # Store high-value turns as semantic memories
            if len(response) > 80 and not command:
    ...
    def send_message(self):
        ...
        def generate_worker():
            try:
                ...
                # ── PATH 1: Full CognitiveEngine (reflection/habits/bus/governance) ──
                _ce = getattr(self, '_cognitive_engine', None)
                _ce_ok = False
                if False and _ce is not None:
                    try:
                        result = _ce.process(
                            user_message,
                            stream=True,
                        ...
                        print(f"[GUI] CognitiveEngine.process failed, falling back: {_ce_err}")
                        result = None

                # ── PATH 2: AgentOrchestrator via _GUIEngineAdapter (11-stage pipeline) ──
                ...
                if not _ce_ok:
                    ...
                    try:
                        ...
                        print(f"[GUI] Orchestrator unavailable, falling back: {_orch_err}")
                        result = None
    ...
    def load_settings(self):
        ...
        if CENTRAL_IMPORTS_AVAILABLE and config:
            try:
                ...
                _batch = config.get('batch_size') or config.get('n_batch')
                if _batch:
        ...
        if SETTINGS_FILE.exists():
            try:
                ...
                self._user_text_color = settings.get(
                ...
                if not (CENTRAL_IMPORTS_AVAILABLE and config):
                    ...
                    self.n_gpu_layers_input.setValue(int(settings.get('n_gpu_layers', 0)))
                    self.max_tokens_input.setValue(int(settings.get('max_tokens', 512)))
                    self.temperature_input.setValue(float(settings.get('temperature', 0.7)))
                _batch = settings.get('batch_size') or settings.get('n_batch')
                if _batch:
                    self.batch_size_input.setValue(int(_batch))
            except Exception as e:
