...
def _eli_fmt_timestamp(value) -> str:
    try:
        if value in (None, ""):
            return ""
...
def discover_gguf_models(base_dirs: Optional[List[Path]] = None) -> List[Dict[str, Any]]:
    ...
    if base_dirs is None:
        base_dirs = [
            ...
            Path(DEFAULT_MODEL_PATH).expanduser().parent,
        ]
        ...
        try:
            ...
            for path in sorted(base.rglob('*.gguf')):
                ...
                if 'mistral' in name_low:
                    ...
                    family = 'phi'
                source = 'bundled' if Path(base).resolve() == Path(BUNDLED_MODEL_DIR).resolve() else 'custom'
                results.append({
                    'name': path.name,
                    'path': str(path),
                    'size_gb': size_gb,
...
class LocalModelManager:
    ...
    def _sync_runtime(self, model_path: str, n_ctx: int, n_threads: int, n_gpu_layers: int) -> dict:
        settings = runtime_settings.load_settings()
        settings["provider"] = settings.get("provider", "custom_gguf") or "custom_gguf"
        settings["model_path"] = model_path
        settings["custom_model_path"] = model_path
        settings.setdefault("bundled_model_path", model_path)
        settings["n_ctx"] = int(n_ctx)
        settings["n_threads"] = int(n_threads)
        settings["cpu_threads"] = int(n_threads)
        settings["n_gpu_layers"] = int(n_gpu_layers)
        settings["gpu_layers"] = int(n_gpu_layers)
        settings["batch_size"] = int(settings.get("batch_size", 512) or 512)
        runtime_settings.save_settings(settings)
        if hasattr(runtime_settings, "apply_env"):
            try:
                runtime_settings.apply_env(settings)
            except Exception:
                pass
        os.environ["ELI_GGUF_MODEL_PATH"] = str(model_path)
        os.environ["ELI_GGUF_N_CTX"] = str(int(n_ctx))
        os.environ["ELI_GGUF_THREADS"] = str(int(n_threads))
        os.environ["ELI_GGUF_N_GPU_LAYERS"] = str(int(n_gpu_layers))
        os.environ["ELI_GGUF_N_BATCH"] = str(int(settings.get("batch_size", 512) or 512))
        if "use_mmap" in settings:
            os.environ["ELI_GGUF_USE_MMAP"] = "1" if settings.get("use_mmap") else "0"
        if "use_mlock" in settings:
            os.environ["ELI_GGUF_USE_MLOCK"] = "1" if settings.get("use_mlock") else "0"
        return settings
    ...
    def load_model(self, model_path: str, n_ctx: int = 4096,
        ...
        try:
            ...
            if not path_obj.exists():
                ...
                self.is_loaded = False
                return False
            settings = self._sync_runtime(str(path_obj), n_ctx, n_threads, n_gpu_layers)
            batch_size = int(settings.get("batch_size", 512) or 512)
            print(f"🔄 Loading model: {path_obj.name}")
            print(f"   Size: {path_obj.stat().st_size / (1024**3):.2f} GB")
            print(f"   Params: ctx={int(n_ctx)}, gpu_layers={int(n_gpu_layers)}, batch={batch_size}, threads={int(n_threads)}")
            if gguf_inference is None:
                self.load_error = "gguf_inference module unavailable"
                self.is_loaded = False
                return False
            gguf_inference.unload_model()
            self.model = gguf_inference.load_model()
            if self.model is None:
                getter = getattr(gguf_inference, "get_last_error", None)
                self.load_error = getter() if callable(getter) else "Failed to load GGUF model"
                self.is_loaded = False
                gc.collect()
                return False
            self.is_loaded = True
    ...
    def chat(self, messages: List[Dict[str, str]], max_tokens: int = 512,
        ...
        try:
            system_msg = next((m.get("content", "") for m in messages if m.get("role") == "system"), ELI_SYSTEM_PROMPT)
            user_msg = "\n".join(m.get("content", "") for m in messages if m.get("role") == "user")
            return gguf_inference.chat_completion(user_msg, system=system_msg, max_tokens=max_tokens, temperature=temperature)
        except Exception as e:
    ...
    def chat_stream(self, messages, max_tokens=1024, temperature=0.7):
        ...
        try:
            system_msg = next((m.get("content", "") for m in messages if m.get("role") == "system"), ELI_SYSTEM_PROMPT)
            user_msg = "\n".join(m.get("content", "") for m in messages if m.get("role") == "user")
            for chunk in gguf_inference.generate(prompt=user_msg, system=system_msg, max_tokens=max_tokens, temperature=temperature, stream=True):
...
class EliMainWindow(QMainWindow):
    ...
    def create_settings_tab(self):
        ...
        self.batch_size_input.setRange(32, 4096)
        self.batch_size_input.setValue(512)
        self.batch_size_input.setSingleStep(32)
        runtime_layout.addRow("Batch Size:", self.batch_size_input)
        ...
        self.user_text_color_combo = QComboBox()
        for label, color in USER_TEXT_COLOR_OPTIONS.items():
            self.user_text_color_combo.addItem(f"{label} ({color})", color)
        app_layout.addRow("User text color:", self.user_text_color_combo)
    ...
    def get_selected_user_text_color(self) -> str:
        if hasattr(self, 'user_text_color_combo'):
            color = self.user_text_color_combo.currentData()
            if color:
                return str(color)
        try:
            settings = runtime_settings.load_settings()
            return str(settings.get('user_text_color', '#6CB6FF') or '#6CB6FF')
        except Exception:
            return '#6CB6FF'
    ...
    def send_message(self):
        ...
        user_color = self.get_selected_user_text_color() if hasattr(self, "get_selected_user_text_color") else "#6CB6FF"
        safe_label = html.escape(str(user_label))
        safe_message = html.escape(str(user_message)).replace("\n", "<br>")
        self.chat_display.append(f'\n<b><span style="color:{user_color};">🧑 {safe_label} [{now_hms()}]:</span></b><br><span style="color:{user_color};">{safe_message}</span><br>')
...
def refresh_memory_stats(self):
    ...
    try:
        if hasattr(mem, "get_dashboard_counts"):
            counts = dict(mem.get_dashboard_counts() or {})
            by_kind = counts.get("by_kind") or counts.get("by_type") or {}
            lines = [
                f"Processed Memories: {int(counts.get('processed_memories', counts.get('raw_memories', 0)) or 0)}",
                f"Raw Memories: {int(counts.get('raw_memories', counts.get('total_memories', 0)) or 0)}",
                f"Conversation Turns: {int(counts.get('conversation_turns', 0) or 0)}",
                f"Conversations: {int(counts.get('conversations', 0) or 0)}",
                f"Habit Rules: {int(counts.get('habit_rules', 0) or 0)}",
                f"Habit Events: {int(counts.get('habit_events', 0) or 0)}",
                f"Observations: {int(counts.get('observations', 0) or 0)}",
                f"Failures: {int(counts.get('failures', 0) or 0)}",
                f"Improvements: {int(counts.get('improvements', 0) or 0)}",
                "",
                "By Type:",
            ]
            for kind, value in sorted((by_kind or {}).items(), key=lambda kv: (-int(kv[1]), str(kv[0]))):
...
def show_recent_memories(self):
    ...
    if mem is None:
        try:
            ...
            self.memory_results_display.append(f"<i>Central memory unavailable: {html.escape(str(e))}</i>")
            return

    try:
        if hasattr(mem, "get_recent_processed_memories"):
            results = mem.get_recent_processed_memories(limit=20)
        else:
            results = []
    except Exception as e:
        self.memory_results_display.append(f"<i>Error loading processed memories: {html.escape(str(e))}</i>")
        return
    ...
    for i, row in enumerate(results, 1):
        ts = _eli_fmt_timestamp(row.get("timestamp", ""))
        kind = html.escape(str(row.get("kind", "note")))
        text = html.escape(str(row.get("text", "")))
        tags = html.escape(str(row.get("tags", "")))
        source = html.escape(str(row.get("source", "")))
        self.memory_results_display.append(
            f"<b>{i}. [{ts}] ({kind})</b><br>{text}<br><i>Tags: {tags}</i><br><i>Source: {source}</i><br><br>"
        )
    ...
    def load_settings(self):
        ...
        try:
            ...
            if hasattr(self, 'n_threads_input'):
                self.n_threads_input.setValue(int(settings.get('cpu_threads', settings.get('n_threads', 8))))
            if hasattr(self, 'n_gpu_layers_input'):
                self.n_gpu_layers_input.setValue(int(settings.get('gpu_layers', settings.get('n_gpu_layers', 10))))
            if hasattr(self, 'batch_size_input'):
                self.batch_size_input.setValue(int(settings.get('batch_size', 512)))
            ...
            if hasattr(self, 'user_text_color_combo'):
                color_value = str(settings.get('user_text_color', '#6CB6FF') or '#6CB6FF')
                idx = self.user_text_color_combo.findData(color_value)
                if idx >= 0:
                    self.user_text_color_combo.setCurrentIndex(idx)
    ...
    def save_settings(self, silent: bool = False):
        ...
        n_threads = self.n_threads_input.value()
        n_gpu_layers = self.n_gpu_layers_input.value()
        batch_size = self.batch_size_input.value() if hasattr(self, 'batch_size_input') else 512
        ...
        user_text_color = self.get_selected_user_text_color()
        ...
        try:
            ...
            print(f"⚠️ Failed to save runtime settings: {e}")

        if CENTRAL_IMPORTS_AVAILABLE and config:
            try:
                config.set('provider', provider)
                if provider != 'ollama':
                    if hasattr(config, 'set_gguf_model_path'):
                        config.set_gguf_model_path(model_path)
                    else:
                        config.set('gguf_model_path', model_path)
                config.set('bundled_model_path', bundled_path)
                config.set('ollama_host', ollama_host)
                config.set('ollama_model', ollama_model)
                if hasattr(config, 'set_gguf_n_ctx'):
                    config.set_gguf_n_ctx(n_ctx)
                else:
                    config.set('gguf_n_ctx', n_ctx)
                if hasattr(config, 'set_n_threads'):
                    config.set_n_threads(n_threads)
                else:
                    config.set('n_threads', n_threads)
                if hasattr(config, 'set_gguf_n_gpu_layers'):
                    config.set_gguf_n_gpu_layers(n_gpu_layers)
                else:
                    config.set('gguf_n_gpu_layers', n_gpu_layers)
                if hasattr(config, 'set_gguf_n_batch'):
                    config.set_gguf_n_batch(batch_size)
                else:
                    config.set('gguf_n_batch', batch_size)
                if hasattr(config, 'set_num_predict'):
                    config.set_num_predict(max_tokens)
                else:
                    config.set('num_predict', max_tokens)
                if hasattr(config, 'set_temperature'):
                ...
                config.set('user_text_color', user_text_color)
                if gguf_inference:
                    gguf_inference.unload_model()
            except Exception as e:
        ...
        try:
            ...
            if not silent:
                QMessageBox.information(self, 'Success', 'Settings saved to both runtime settings and local file!')
    ...
    def _append_log(self, message):
        ...
        for attr in ("log_display", "logs_display", "system_log", "output_display", "status_display"):
    ...
    def closeEvent(self, event):
        ...
        if self.auto_save_checkbox.isChecked() and self.conversation_history:
            try:
                ...
                filename = CONVERSATIONS_DIR / f"conversation_{timestamp}.json"
                with open(filename, 'w', encoding='utf-8') as f:
                    json.dump({"timestamp": now_timestamp(), "messages": self.conversation_history}, f, indent=2, ensure_ascii=False)
                saved_name = filename.name
                self.status_signal.emit(f"Conversation stored: {saved_name}")
                print(f"[GUI] Conversation stored on close: {filename}")
            except Exception as e:
                print(f"[GUI] Failed to store conversation on close: {e}")
        event.accept()

def _eli_append_log_bound_fallback(self, message):
    text = str(message)
...
try:
    EliMainWindow._append_log = _eli_append_log_bound_fallback
except Exception:
    pass


def _eli_slot_text(payload) -> str:
    try:
...
def _eli_generic_slot_factory(name):
    def _slot(self, *args, **kwargs):
        payload = args[0] if args else ""
        text = _eli_slot_text(payload)

        try:
            if name == "_append_log":
                return _eli_append_log_bound_fallback(self, text)

            if name == "_update_status":
            ...
            if name in ("_update_chat", "_append_chat", "_update_chat_display"):
                w = getattr(self, "chat_display", None)
                if w is not None:
                    try:
                        if hasattr(w, "append"):
                            w.append(text)
            ...
            if name == "_update_proactive":
                for attr in ("proactive_display", "proactive_results_display", "suggestions_display"):
            ...
            if name == "_update_failures_display":
                w = getattr(self, "failures_display", None)
                if w is not None:
                    try:
                        if hasattr(w, "setPlainText"):
                            w.setPlainText(text)
            ...
            if name == "_update_improvements_display":
                w = getattr(self, "improvements_display", None)
                if w is not None:
                    try:
                        if hasattr(w, "setPlainText"):
                            w.setPlainText(text)
            ...
            _eli_append_log_bound_fallback(self, f"[AUTO-SLOT:{name}] {text}")
        except Exception as e:
            try:
                print(f"[AUTO-SLOT:{name}] failed: {e}")
            except Exception:
                pass
...
def _eli_bind_missing_qt_slots():
    import re
    ...
    connected = sorted(set(re.findall(r'\.connect\(self\.(\w+)', src)))

    preferred = {
        "_append_log": _eli_append_log_bound_fallback,
        "_update_status": _eli_generic_slot_factory("_update_status"),
        "_update_proactive": _eli_generic_slot_factory("_update_proactive"),
        "_update_failures_display": _eli_generic_slot_factory("_update_failures_display"),
        "_update_improvements_display": _eli_generic_slot_factory("_update_improvements_display"),
        "_update_chat": _eli_generic_slot_factory("_update_chat"),
    }

    for name, fn in preferred.items():
        if not hasattr(EliMainWindow, name):
            setattr(EliMainWindow, name, fn)

    for name in connected:
        if not hasattr(EliMainWindow, name):
            setattr(EliMainWindow, name, _eli_generic_slot_factory(name))


try:
    _eli_bind_missing_qt_slots()
except Exception as _eli_slot_bind_error:
    try:
        print(f"[AUTO-SLOT-BIND] failed: {_eli_slot_bind_error}")
    except Exception:
        pass


def _eli_set_text_value(widget, value):
    try:
        if widget is None:
            return
...
def _eli_set_bool_value(widget, value):
    try:
        if widget is None:
            return
...
def _eli_set_num_value(widget, value):
    try:
        if widget is None:
            return
...
def _eli_set_combo_value(widget, value):
    try:
        if widget is None:
            return
...
def _eli_apply_runtime_settings_to_window(self, cfg):
    cfg = dict(cfg or {})
    self._runtime_settings_cache = cfg

    # simple attrs
    try:
        self.current_theme = cfg.get("theme", getattr(self, "current_theme", "dark"))
    except Exception:
        pass

    try:
        self.user_name = cfg.get("user_name", getattr(self, "user_name", "USER*")) or "USER*"
    except Exception:
        pass

    try:
        self.user_text_color = cfg.get("user_text_color", getattr(self, "user_text_color", "#4A90E2")) or "#4A90E2"
    except Exception:
        pass

    # text/path fields
    text_fields = {
        "model_path_input": cfg.get("model_path", ""),
        "custom_model_path_input": cfg.get("custom_model_path", ""),
        "bundled_model_path_input": cfg.get("bundled_model_path", ""),
        "ollama_host_input": cfg.get("ollama_host", ""),
        "ollama_model_input": cfg.get("ollama_model", ""),
        "persona_file_input": cfg.get("persona_file", ""),
        "user_name_input": cfg.get("user_name", ""),
    }
    for attr, value in text_fields.items():
        _eli_set_text_value(getattr(self, attr, None), value)

    # combos/dropdowns
    combo_fields = {
        "provider_combo": cfg.get("provider", ""),
        "quick_mode_combo": cfg.get("quick_mode", ""),
        "theme_combo": cfg.get("theme", ""),
        "user_text_color_combo": cfg.get("user_text_color", ""),
        "user_color_combo": cfg.get("user_text_color", ""),
        "user_text_colour_combo": cfg.get("user_text_color", ""),
    }
    for attr, value in combo_fields.items():
        _eli_set_combo_value(getattr(self, attr, None), value)

    # numeric controls
    num_fields = {
        "n_ctx_spin": int(cfg.get("n_ctx", 8192)),
        "max_tokens_spin": int(cfg.get("max_tokens", 4096)),
        "temperature_spin": float(cfg.get("temperature", 0.7)),
        "top_p_spin": float(cfg.get("top_p", 0.95)),
        "top_k_spin": int(cfg.get("top_k", 40)),
        "repeat_penalty_spin": float(cfg.get("repeat_penalty", 1.1)),
        "gpu_layers_spin": int(cfg.get("gpu_layers", cfg.get("n_gpu_layers", 0))),
        "cpu_threads_spin": int(cfg.get("cpu_threads", cfg.get("n_threads", 8))),
        "batch_size_spin": int(cfg.get("batch_size", 512)),
        "n_threads_spin": int(cfg.get("n_threads", 8)),
        "n_gpu_layers_spin": int(cfg.get("n_gpu_layers", cfg.get("gpu_layers", 0))),
    }
    for attr, value in num_fields.items():
        _eli_set_num_value(getattr(self, attr, None), value)

    # checkboxes
    bool_fields = {
        "auto_save_checkbox": cfg.get("auto_save", True),
        "auto_load_checkbox": cfg.get("auto_load", True),
        "auto_speak_checkbox": cfg.get("auto_speak", False),
        "mic_enabled_checkbox": cfg.get("mic_enabled", False),
        "mic_checkbox": cfg.get("mic_enabled", False),
        "log_to_file_checkbox": cfg.get("log_to_file", False),
        "use_mmap_checkbox": cfg.get("use_mmap", True),
        "use_mlock_checkbox": cfg.get("use_mlock", False),
    }
    for attr, value in bool_fields.items():
        _eli_set_bool_value(getattr(self, attr, None), value)

    # apply theme if the method exists
    try:
    ...
    try:
        if hasattr(self, "statusBar"):
            sb = self.statusBar()
            if sb is not None:
                sb.showMessage(f"Settings loaded from {runtime_settings.SETTINGS_FILE}", 4000)
    except Exception:
        pass


def _eli_load_settings_bound_fallback(self):
    try:
        cfg = runtime_settings.load_settings()
    except Exception as e:
        cfg = {}
        try:
            print(f"[LOAD_SETTINGS] runtime_settings.load_settings failed: {e}")
        except Exception:
            pass

    _eli_apply_runtime_settings_to_window(self, cfg)
    return cfg


try:
    if not hasattr(EliMainWindow, "load_settings"):
        EliMainWindow.load_settings = _eli_load_settings_bound_fallback
except Exception as _eli_load_bind_error:
    try:
        print(f"[LOAD_SETTINGS] bind failed: {_eli_load_bind_error}")
    except Exception:
        pass


def _eli_fallback_apply_theme(self):
    theme = str(getattr(self, "current_theme", "dark") or "dark").lower()
    user_color = str(getattr(self, "user_text_color", "#4A90E2") or "#4A90E2")
    ...
    try:
        if hasattr(self, "chat_input") and self.chat_input is not None:
            self.chat_input.setStyleSheet(f"color: {user_color};")
    except Exception:
        pass


def _eli_fallback_toggle_theme(self):
    self.current_theme = "light" if str(getattr(self, "current_theme", "dark")).lower() == "dark" else "dark"
    self.apply_theme()


def _eli_rebind_gui_methods():
    wanted = [
        "_append_log",
        "_update_status",
        "_update_failures_display",
        "_update_improvements_display",
        "_update_insights_display",
        "_update_summary_display",
        "_update_suggestions_display",
        "_update_proactive",
        "_si_analyze_failures",
        "_si_run_cycle",
        "_speak_last_response",
        "_stt_toggle",
        "_on_stt_transcript",
        "_on_auto_speak_toggled",
        "load_settings",
        "save_settings",
        "apply_theme",
        "toggle_theme",
        "refresh_memory_stats",
        "refresh_model_sources",
    ...
    for name in wanted:
        if hasattr(EliMainWindow, name):
            continue
        fn = globals().get(name)
        if callable(fn):
            setattr(EliMainWindow, name, fn)

    if not hasattr(EliMainWindow, "apply_theme"):
        EliMainWindow.apply_theme = _eli_fallback_apply_theme

    if not hasattr(EliMainWindow, "toggle_theme"):
        EliMainWindow.toggle_theme = _eli_fallback_toggle_theme


try:
    _eli_rebind_gui_methods()
except Exception as _eli_rebind_error:
    try:
        print(f"[GUI REBIND] failed: {_eli_rebind_error}")
    except Exception:
        pass


def recommend_model_setup(window=None):
    """
    Safe first-run recommender shim.
    Prevents startup warnings/crashes if the original helper was lost during
    class-boundary / patch churn.
    """
    cfg = {}
    try:
        from eli.core import runtime_settings as _rs
        cfg = dict(_rs.load_settings() or {})
    except Exception:
        cfg = {}

    if window is not None:
        fn = getattr(window, "detect_optimal_settings", None)
        if callable(fn):
            try:
                out = fn()
    ...
    return {
        "provider": cfg.get("provider", "custom_gguf"),
        "model_path": model_path,
        "n_ctx": int(cfg.get("n_ctx", 8192) or 8192),
        "n_threads": int(cfg.get("cpu_threads", cfg.get("n_threads", 8)) or 8),
        "n_gpu_layers": int(cfg.get("gpu_layers", cfg.get("n_gpu_layers", 0)) or 0),
        "batch_size": int(cfg.get("batch_size", 512) or 512),
        "temperature": float(cfg.get("temperature", 0.7) or 0.7),
    }


def _eli_method_binding_audit():
    """
    Rebind real top-level functions back onto EliMainWindow if they were
    accidentally dedented out of the class, but do not overwrite existing
    ...
    for name in critical:
        current = getattr(EliMainWindow, name, None)
        top = globals().get(name)

        current_name = getattr(current, "__name__", "")
        current_qual = getattr(current, "__qualname__", "")

        is_generic = (
            current_name == "_slot"
            or "_eli_generic_slot_factory" in current_qual
        )

        if callable(top) and (current is None or is_generic):
            setattr(EliMainWindow, name, top)


try:
    _eli_method_binding_audit()
except Exception as _eli_bind_err:
    try:
        print(f"[GUI BIND AUDIT] {_eli_bind_err}")
    except Exception:
        pass



# --- ELI functional fallback patch v4 ---
def recommend_model_setup(*args, **kwargs):
    window = kwargs.get("window")
    if window is None:
        for obj in args:
            if hasattr(obj, "detect_optimal_settings"):
                window = obj
                break
...
def _eli_is_generic_slot(fn):
    return callable(fn) and (
        getattr(fn, "__name__", "") == "_slot"
        or "_eli_generic_slot_factory" in getattr(fn, "__qualname__", "")
    )


def _eli_get_widget_value(widget, default=None):
    if widget is None:
        return default
    for meth in ("value", "text", "currentText", "toPlainText"):
        if hasattr(widget, meth):
            try:
                return getattr(widget, meth)()
            except Exception:
                pass
...
def _eli_set_text_into_widgets(self, text, *attrs):
    payload = str(text)
    for attr in attrs:
        widget = getattr(self, attr, None)
        if widget is None:
        ...
        try:
            ...
            if hasattr(widget, "setText"):
                ...
                return True
            if hasattr(widget, "clear") and hasattr(widget, "append"):
                widget.clear()
                widget.append(payload)
                return True
            if hasattr(widget, "appendPlainText"):
                widget.appendPlainText(payload)
                return True
            if hasattr(widget, "append"):
                widget.append(payload)
                return True
...
def _eli_fallback_update_status(self, message):
    text = str(message)
    _eli_set_text_into_widgets(self, text, "status_display", "status_label", "model_status_label")
    try:
        sb = self.statusBar() if hasattr(self, "statusBar") else None
        if sb is not None:
            sb.showMessage(text[:240], 5000)
    except Exception:
        pass


def _eli_fallback_update_proactive(self, data):
    if isinstance(data, dict):
        lines = [f"{k}: {v}" for k, v in data.items()]
        text = "\n".join(lines)
    else:
        text = str(data)
    if not _eli_set_text_into_widgets(self, text, "proactive_display", "insights_display", "summary_display"):
        _eli_fallback_update_status(self, text)


def _eli_fallback_toggle_theme(self):
    current = getattr(self, "current_theme", "dark")
    self.current_theme = "light" if current == "dark" else "dark"
    fn = getattr(self, "apply_theme", None)
    if callable(fn):
        fn()
    try:
        from eli.core import runtime_settings as _rs
        ...
        _rs.save_settings(cfg)
    except Exception:
        pass


def _eli_fallback_save_settings(self, *args, **kwargs):
    from eli.core import runtime_settings as _rs

    cfg = dict(_rs.load_settings() or {})

    provider_widget = getattr(self, "provider_combo", None)
    provider_text = str(_eli_get_widget_value(provider_widget, cfg.get("provider", "custom_gguf")) or "").lower()
    if "ollama" in provider_text:
        cfg["provider"] = "ollama"
    elif "bundled" in provider_text:
        cfg["provider"] = "bundled_gguf"
    else:
        cfg["provider"] = "custom_gguf"

    cfg["model_path"] = str(_eli_get_widget_value(getattr(self, "model_path_input", None), cfg.get("model_path", "")) or "")
    cfg["custom_model_path"] = str(_eli_get_widget_value(getattr(self, "model_path_input", None), cfg.get("custom_model_path", "")) or cfg["model_path"])
    cfg["bundled_model_path"] = str(cfg.get("bundled_model_path") or cfg["model_path"])
    cfg["ollama_host"] = str(_eli_get_widget_value(getattr(self, "ollama_host_input", None), cfg.get("ollama_host", "http://localhost:11434")) or "http://localhost:11434")
    cfg["ollama_model"] = str(
        _eli_get_widget_value(getattr(self, "ollama_model_combo", None),
        _eli_get_widget_value(getattr(self, "ollama_model_input", None), cfg.get("ollama_model", ""))) or ""
    )

    cfg["n_ctx"] = int(_eli_get_widget_value(getattr(self, "n_ctx_spin", None),
                    _eli_get_widget_value(getattr(self, "context_spin", None), cfg.get("n_ctx", 8192))) or 8192)
    cfg["cpu_threads"] = int(_eli_get_widget_value(getattr(self, "cpu_threads_spin", None),
                           _eli_get_widget_value(getattr(self, "threads_spin", None),
                           _eli_get_widget_value(getattr(self, "n_threads_spin", None), cfg.get("cpu_threads", cfg.get("n_threads", 8))))) or 8)
    cfg["n_threads"] = int(cfg["cpu_threads"])
    cfg["gpu_layers"] = int(_eli_get_widget_value(getattr(self, "gpu_layers_spin", None),
                          _eli_get_widget_value(getattr(self, "n_gpu_layers_spin", None), cfg.get("gpu_layers", cfg.get("n_gpu_layers", 0)))) or 0)
    cfg["n_gpu_layers"] = int(cfg["gpu_layers"])
    cfg["batch_size"] = int(_eli_get_widget_value(getattr(self, "batch_spin", None),
                          _eli_get_widget_value(getattr(self, "batch_size_spin", None), cfg.get("batch_size", 512))) or 512)
    cfg["max_tokens"] = int(_eli_get_widget_value(getattr(self, "max_tokens_spin", None), cfg.get("max_tokens", 4096)) or 4096)
    cfg["temperature"] = float(_eli_get_widget_value(getattr(self, "temperature_spin", None), cfg.get("temperature", 0.7)) or 0.7)

    cfg["theme"] = str(_eli_get_widget_value(getattr(self, "theme_combo", None), getattr(self, "current_theme", cfg.get("theme", "dark"))) or "dark")
    cfg["user_name"] = str(_eli_get_widget_value(getattr(self, "user_name_input", None), cfg.get("user_name", "")) or "")
    cfg["user_text_color"] = str(_eli_get_widget_value(getattr(self, "user_text_color_combo", None), cfg.get("user_text_color", "#F2F2F2")) or "#F2F2F2")
    cfg["auto_save"] = bool(_eli_get_widget_value(getattr(self, "auto_save_checkbox", None), cfg.get("auto_save", True)))
    cfg["auto_load"] = bool(_eli_get_widget_value(getattr(self, "auto_load_checkbox", None), cfg.get("auto_load", True)))

    _rs.save_settings(cfg)
    if not kwargs.get("silent", False):
        _eli_fallback_update_status(self, f"Settings saved to {_rs.SETTINGS_FILE}")


def _eli_fallback_analyze_context(self):
    mem = getattr(self, "_central_memory", None)
    lines = []

    if mem and hasattr(mem, "get_dashboard_counts"):
        try:
            counts = mem.get_dashboard_counts()
            lines.append("Memory dashboard:")
            for k, v in counts.items():
                lines.append(f"- {k}: {v}")
        except Exception as e:
            lines.append(f"Dashboard error: {e}")

    if mem and hasattr(mem, "get_recent_observations"):
        try:
            obs = mem.get_recent_observations(limit=5)
            if obs:
                lines.append("")
                lines.append("Recent observations:")
                for item in obs:
                    lines.append(f"- [{item.get('category','system')}] {item.get('observation','')}")
        except Exception as e:
            lines.append(f"Observation error: {e}")

    if not lines:
        lines.append("No context analysis available yet.")

    _eli_set_text_into_widgets(self, "\n".join(lines), "summary_display", "insights_display", "proactive_display")


def _eli_fallback_generate_suggestions(self):
    mem = getattr(self, "_central_memory", None)
    lines = []

    if mem and hasattr(mem, "get_recent_processed_memories"):
        try:
            recent = mem.get_recent_processed_memories(limit=5)
            for item in recent:
                kind = item.get("kind", "note")
                text = str(item.get("text", "")).strip().replace("\n", " ")
                if text:
                    lines.append(f"- Revisit {kind}: {text[:120]}")
        except Exception as e:
            lines.append(f"- Suggestion generation error: {e}")

    if mem and hasattr(mem, "get_recent_observations"):
        try:
            for item in mem.get_recent_observations(limit=3):
    ...
    if not lines:
        lines.append("- No live suggestions yet. Use the app a bit more to generate patterns.")

    _eli_set_text_into_widgets(self, "\n".join(lines), "suggestions_display", "insights_display", "summary_display")


def _eli_bind_functional_fallbacks():
    bindings = {
        "save_settings": _eli_fallback_save_settings,
        "toggle_theme": _eli_fallback_toggle_theme,
        "_update_status": _eli_fallback_update_status,
        "_update_proactive": _eli_fallback_update_proactive,
        "analyze_context": _eli_fallback_analyze_context,
        "generate_suggestions": _eli_fallback_generate_suggestions,
    }

    for name, fn in bindings.items():
        cur = getattr(EliMainWindow, name, None)
        if cur is None or _eli_is_generic_slot(cur):
            setattr(EliMainWindow, name, fn)


try:
    _eli_bind_functional_fallbacks()
except Exception as _eli_gui_bind_err:
    try:
        print(f"[GUI FALLBACK BIND] {_eli_gui_bind_err}")
    except Exception:
        pass

def _eli_mark_model_ready(self, model_path="", provider="GGUF"):
    try:
    ...
    try:
        if hasattr(self, "model_status_label") and self.model_status_label is not None:
            name = Path(str(model_path)).name if model_path else "Loaded"
    ...
    try:
        if hasattr(self, "status_label") and self.status_label is not None:
            self.status_label.setText("🔒 100% LOCAL - No external connections")
    except Exception:
        pass
    ...
    try:
        if hasattr(self, "statusBar"):
            sb = self.statusBar()
            if sb is not None:
                name = Path(str(model_path)).name if model_path else provider
                sb.showMessage(f"{provider} model ready: {name}", 5000)
    except Exception:
        pass


def _eli_mark_model_failed(self, reason="Model load failed"):
    try:
    ...
    try:
        if hasattr(self, "send_btn") and self.send_btn is not None:
            self.send_btn.setEnabled(False)
            self.send_btn.setText("Load Model")
    except Exception:
        pass
    ...
    try:
        if hasattr(self, "statusBar"):
            sb = self.statusBar()
            if sb is not None:
                sb.showMessage(str(reason)[:200], 5000)
    except Exception:
        pass


try:
    EliMainWindow._eli_mark_model_ready = _eli_mark_model_ready
    EliMainWindow._eli_mark_model_failed = _eli_mark_model_failed
except Exception:
    pass


def _eli_model_backend_loaded(self):
    try:
    ...
    try:
        gi = globals().get("gguf_inference", None)
        if gi is not None:
            for name in ("_model", "_MODEL", "model", "MODEL", "_llm", "llm"):
                try:
                    if getattr(gi, name, None) is not None:
                        return True
                except Exception:
                    pass
    except Exception:
...
def _eli_current_model_name(self):
    try:
        if hasattr(self, "model_path_combo") and self.model_path_combo is not None:
            txt = self.model_path_combo.currentText().strip()
            if txt:
    ...
    try:
        if hasattr(self, "model_path_input") and self.model_path_input is not None:
            txt = self.model_path_input.text().strip()
            if txt:
                return Path(txt).name
    except Exception:
        pass
    ...
    try:
        rs = runtime_settings.load_settings()
        mp = str(rs.get("model_path") or rs.get("custom_model_path") or rs.get("bundled_model_path") or "").strip()
        if mp:
            return Path(mp).name
    except Exception:
        pass
...
def _eli_sync_model_widgets(self):
    loaded = _eli_model_backend_loaded(self)
    model_name = _eli_current_model_name(self)
    ...
    # explicit known attributes first
    candidate_labels = [
        "model_status_label",
        "model_label",
        "chat_model_label",
    ...
    for attr in candidate_labels:
        try:
            w = getattr(self, attr, None)
            if w is not None and hasattr(w, "setText"):
                w.setText(f"🟢 Model: {model_name}" if loaded else "🔴 Model: Not loaded")
        except Exception:
            pass

    # scan all QLabel widgets so we catch the real one even if attr names changed
    try:
        for lab in self.findChildren(QLabel):
            try:
                ...
                continue
            if txt.startswith("🔴 Model:") or txt.startswith("🟢 Model:") or txt.startswith("Model:"):
                try:
                    lab.setText(f"🟢 Model: {model_name}" if loaded else "🔴 Model: Not loaded")
                except Exception:
                    pass
    except Exception:
        pass
    ...
    try:
        ...
        if btn is not None:
            btn.setEnabled(bool(loaded))
            if loaded:
                btn.setText("Send")
            else:
                if btn.text().strip().lower() in {"send", "loading...", "load model"}:
                    btn.setText("Loading...")
    except Exception:
        pass

    # any button stuck on Loading...
    try:
    ...
    try:
        ...
        if sb is not None:
            sb.showMessage(
                f"GGUF ready: {model_name}" if loaded else "Model not loaded",
                3000
            )
    except Exception:
        pass
...
def _eli_install_model_sync_timer(self):
    try:
        if getattr(self, "_eli_model_sync_timer", None) is not None:
            return
    except Exception:
        pass
    ...
    try:
        ...
        timer.timeout.connect(lambda: _eli_sync_model_widgets(self))
        timer.start()
        self._eli_model_sync_timer = timer
    except Exception as e:
        print(f"[MODEL UI SYNC] timer install failed: {e}")


try:
    EliMainWindow._eli_model_backend_loaded = _eli_model_backend_loaded
    EliMainWindow._eli_current_model_name = _eli_current_model_name
    EliMainWindow._eli_sync_model_widgets = _eli_sync_model_widgets
    EliMainWindow._eli_install_model_sync_timer = _eli_install_model_sync_timer
except Exception:
    pass
...
def _eli_fallback_append_chat_response(self, text: str):
    """Robust chat sink for streamed tokens and full responses on the GUI thread."""
    ...
    if text == "__STREAM_START__":
        ...
        try:
            ...
            try:
                ...
                print(f"[_append_chat_response:start] {e}")
        return
    ...
    if getattr(self, '_streaming', False):
        ...
        try:
            ...
            try:
                ...
                print(f"[_append_chat_response:token] {text}")
        return
    ...
    try:
        ...
        try:
            self.chat_display.append(f"<br><b>🤖 ELI [{ts}]:</b><br>{text}<br>")
            self.chat_display.ensureCursorVisible()
        except Exception:
            print(f"[_append_chat_response:full] {e}: {text}")


try:
    EliMainWindow._append_chat_response = _eli_fallback_append_chat_response
except Exception:
    pass
...
╚══════════════════════════════════════════════════════════════════════╝
    ...
    if os.environ.get('QT_STYLE_OVERRIDE', '').strip().lower() == 'adwaita':
    ...
    try:
        window._eli_install_model_sync_timer()
        window._eli_sync_model_widgets()
    except Exception as e:
        print(f"[MODEL UI SYNC] startup hook failed: {e}")
    window.show()
...
# ============================================================
# ELI GUI HOTFIX: display / status / memory panels
# append-only override block
# ============================================================

def _eli_hotfix_now_hms():
    from datetime import datetime
    return datetime.now().strftime("%H:%M:%S")

def _eli_hotfix_user_label(self):
    try:
        import eli.core.runtime_settings as _rs
        settings = _rs.load_settings()
        name = str(settings.get("user_name", "") or "").strip()
        return name if name else "USER*"
    except Exception:
        return "USER*"

def _eli_hotfix_update_status(self, text):
    msg = str(text or "").strip()
    try:
        sb = self.statusBar() if hasattr(self, "statusBar") else None
        if sb is not None:
            sb.showMessage(msg, 5000 if msg else 0)
    except Exception:
        pass

def _eli_hotfix_append_log(self, message):
    text = str(message or "")
    # Never send chat responses to the status bar by default.
    for attr in ("log_display", "logs_display", "system_log", "output_display"):
        widget = getattr(self, attr, None)
        if widget is None:
            continue
        try:
...
def _eli_hotfix_append_chat_response(self, message):
    text = str(message or "").strip()
    if not text:
        return

    # Ignore bogus status-slot leak text
    if text.startswith("[AUTO-SLOT:_append_chat_response]"):
        return

    try:
        ts = _eli_hotfix_now_hms()

        if hasattr(self, "chat_display") and self.chat_display is not None:
            try:
                self.chat_display.append(
                    f"<b>🤖 ELI [{ts}]:</b><br>{text}<br>"
                )
                if hasattr(self.chat_display, "ensureCursorVisible"):
                    self.chat_display.ensureCursorVisible()
            except Exception:
                try:
                    self.chat_display.appendPlainText(f"ELI [{ts}]: {text}")
                except Exception:
                    pass

        # Keep in-memory history aligned
        try:
            if not hasattr(self, "conversation_history") or self.conversation_history is None:
                self.conversation_history = []
            self.conversation_history.append({"role": "assistant", "content": text, "timestamp": ts})
            self._last_eli_response = text
        except Exception:
            pass

        _eli_hotfix_update_status(self, "Idle")
    except Exception as e:
        print(f"[HOTFIX append_chat_response] {e}")

def _eli_hotfix_refresh_memory_stats(self):
    try:
        ...
        target = getattr(self, "memory_stats_display", None)
        if target is not None:
            if hasattr(target, "setPlainText"):
                target.setPlainText("\n".join(lines))
            elif hasattr(target, "setText"):
                target.setText("\n".join(lines))
        _eli_hotfix_update_status(self, "Memory stats refreshed")
    except Exception as e:
        target = getattr(self, "memory_stats_display", None)
        if target is not None:
            try:
                target.setPlainText(f"Failed to refresh memory stats: {e}")
            except Exception:
                pass
        print(f"[HOTFIX refresh_memory_stats] {e}")

def _eli_hotfix_show_recent_memories(self):
    try:
        from eli.brain.memory.memory import Memory
        from eli.core.paths import PATHS
        from datetime import datetime
        ...
        rows = mem.get_recent_processed_memories(limit=20) if hasattr(mem, "get_recent_processed_memories") else []

        box = getattr(self, "memory_results_display", None)
        if box is None:
            return
        ...
        if not rows:
            if hasattr(box, "append"):
                box.append("<i>No processed memories found.</i>")
            elif hasattr(box, "appendPlainText"):
                box.appendPlainText("No processed memories found.")
            return

        out = ["<b>Recent Processed Memories:</b><br><br>"]
        for idx, row in enumerate(rows, 1):
            ts = row.get("timestamp")
            try:
                ts_fmt = datetime.fromtimestamp(float(ts)).strftime("%Y-%m-%d %H:%M:%S")
            except Exception:
                ts_fmt = str(ts)
            ...
            out.append(f"<b>{idx}. [{ts_fmt}] ({kind})</b><br>")
            out.append(f"{text}<br>")
            if tags:
                out.append(f"<i>Tags: {tags}</i><br>")
            if source:
                out.append(f"<i>Source: {source}</i><br>")
            out.append("<br>")

        html = "".join(out)
        if hasattr(box, "append"):
            box.append(html)
        elif hasattr(box, "setHtml"):
            box.setHtml(html)
        elif hasattr(box, "appendPlainText"):
            box.appendPlainText(html)

        _eli_hotfix_update_status(self, "Loaded recent processed memories")
    except Exception as e:
        box = getattr(self, "memory_results_display", None)
        if box is not None:
            try:
                box.clear()
                box.append(f"<i>Error loading processed memories: {e}</i>")
            except Exception:
                pass
        print(f"[HOTFIX show_recent_memories] {e}")

def _eli_hotfix_si_analyze_failures(self):
    try:
        from eli.brain.memory.memory import Memory
        from eli.core.paths import PATHS
        ...
        rows = mem.get_recent_failures(limit=20) if hasattr(mem, "get_recent_failures") else []

        lines = [f"Failures: {len(rows)}", ""]
        for row in rows:
            lines.append(
                f"- [{row.get('timestamp')}] {row.get('command') or row.get('user_input')}: {row.get('error')}"
            )

        text = "\n".join(lines).strip()
        if hasattr(self, "self_improve_failures_signal"):
            self.self_improve_failures_signal.emit(text)
        else:
            target = getattr(self, "failures_display", None)
            if target is not None and hasattr(target, "setPlainText"):
                target.setPlainText(text)

        _eli_hotfix_update_status(self, "Failure analysis complete")
    except Exception as e:
        msg = f"Failure analysis error: {e}"
        if hasattr(self, "self_improve_failures_signal"):
            self.self_improve_failures_signal.emit(msg)
        print(f"[HOTFIX _si_analyze_failures] {e}")

def _eli_hotfix_si_run_cycle(self):
    try:
        from eli.brain.memory.memory import Memory
        from eli.core.paths import PATHS
        ...
        fails = mem.get_recent_failures(limit=10) if hasattr(mem, "get_recent_failures") else []
        imps = mem.get_recent_improvements(limit=20) if hasattr(mem, "get_recent_improvements") else []
        obs = mem.get_recent_observations(limit=10) if hasattr(mem, "get_recent_observations") else []
        ...
        if imps:
            ...
            for row in imps[:10]:
                lines.append(
                    f"- [{row.get('category')}/{row.get('area')}] {row.get('description')} ({row.get('status')})"
                )
            lines.append("")
        ...
        if obs:
            lines.append("=== Recent Observations ===")
            for row in obs[:5]:
                lines.append(f"- [{row.get('category')}] {row.get('observation')}")
            lines.append("")

        lines.append("--- Cycle complete ---")
        text = "\n".join(lines)

        if hasattr(self, "self_improve_improvements_signal"):
            self.self_improve_improvements_signal.emit(text)
        else:
            target = getattr(self, "improvements_display", None)
            if target is not None and hasattr(target, "setPlainText"):
                target.setPlainText(text)

        _eli_hotfix_update_status(self, "Improvement cycle complete")
    except Exception as e:
        msg = f"Improvement cycle error: {e}"
        if hasattr(self, "self_improve_improvements_signal"):
            self.self_improve_improvements_signal.emit(msg)
        print(f"[HOTFIX _si_run_cycle] {e}")

def _eli_hotfix_close_event(self, event):
    try:
        if getattr(self, "conversation_history", None):
            try:
                if hasattr(self, "save_conversation"):
                    self.save_conversation()
            except Exception as e:
                print(f"[HOTFIX closeEvent save_conversation] {e}")
        _eli_hotfix_update_status(self, "Conversation stored")
    except Exception as e:
        print(f"[HOTFIX closeEvent] {e}")
    try:
        event.accept()
    except Exception:
        pass
...
try:
    EliMainWindow._append_chat_response = _eli_hotfix_append_chat_response
    EliMainWindow._append_log = _eli_hotfix_append_log
    EliMainWindow._update_status = _eli_hotfix_update_status
    EliMainWindow.refresh_memory_stats = _eli_hotfix_refresh_memory_stats
    EliMainWindow.show_recent_memories = _eli_hotfix_show_recent_memories
    EliMainWindow._si_analyze_failures = _eli_hotfix_si_analyze_failures
    EliMainWindow._si_run_cycle = _eli_hotfix_si_run_cycle
    EliMainWindow.closeEvent = _eli_hotfix_close_event
except Exception as _eli_gui_hotfix_bind_error:
    print(f"[ELI GUI HOTFIX] bind failed: {_eli_gui_hotfix_bind_error}")
