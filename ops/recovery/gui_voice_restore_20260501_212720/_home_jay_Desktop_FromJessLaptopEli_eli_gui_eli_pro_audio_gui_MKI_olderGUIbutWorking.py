...
_os.environ["ALSA_PCM_CARD"] = "pipewire"
# Redirect stderr briefly during pyaudio/speechrecognition init
import ctypes
try:
...
# --- Add project's src directory to path FIRST ---
# This ensures the 'eli' package is importable.
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))

# Session memory bridge (chat_history)
try:
    from eli.brain.memory.memory_service import (
...
# Now import ELI modules (path is already set)
...
try:
    ...
    import eli.brain.authority.state as eli_state
    from eli.tools.automation.executor_enhanced import proactive_status, proactive_start, proactive_stop
    from eli.tools.io.audio_stt import start_audio_listening, stop_audio_listening, listen_for_command
    from eli.brain.cognition.chat_model import chat_response_stream
...
class ProactiveDockWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.pattern_count_lbl = QLabel("Awaiting live proactive analysis...")
    ...
    def refresh(self):
        if not self.daemon:
            try:
                from eli.brain.proactive.proactive_daemon import get_daemon
        ...
        try:
            ...
            self.patterns_text.setPlainText('\n'.join(lines) if lines else "(0 live patterns returned by daemon)")
            ...
            self.suggestions_text.setPlainText('\n\n'.join(sugs) if sugs else "(0 actionable suggestions from current daemon output)")
        except Exception as e:
            self.patterns_text.setPlainText(f"Error: {e}")

        # Health – FIXED: query both memory_db and user_db
        try:
            import sqlite3
            from eli.core.paths import PATHS
            from pathlib import Path

            # ----- Memory DB (eli_memory.sqlite3) -----
            mem_db = PATHS.memory_db
            mem_entries = 0
            if mem_db and Path(mem_db).exists():
                con = sqlite3.connect(str(mem_db))
                cur = con.cursor()
                cur.execute("SELECT name FROM sqlite_master WHERE type='table'")
                tables = [r[0] for r in cur.fetchall()]
                def table_count(tbl_names):
                    ...
                    return 0
                mem_entries = table_count(["memories", "memory", "eli_memory", "mem"])
                con.close()
            else:
                mem_entries = 0

            # ----- User DB (user.sqlite3) -----
            user_db = PATHS.user_db
            conv_total = 0
            distinct_sessions = 0
            current_session_turns = 0
            tables = []  # for display fallback
            if user_db and Path(user_db).exists():
                con = sqlite3.connect(str(user_db))
                cur = con.cursor()
                cur.execute("SELECT name FROM sqlite_master WHERE type='table'")
                tables = [r[0] for r in cur.fetchall()]
                # Count conversation turns from both possible tables
                if "conversation_turns" in tables:
                    cur.execute("SELECT COUNT(*) FROM conversation_turns")
                    conv_total += int(cur.fetchone()[0] or 0)
                if "conversations" in tables:
                    cur.execute("SELECT COUNT(*) FROM conversations")
                    conv_total += int(cur.fetchone()[0] or 0)
                # Distinct sessions
                for tbl in ("conversations", "conversation_turns"):
                    if tbl in tables:
                        try:
                            cur.execute(
                                f"SELECT COUNT(DISTINCT session_id) FROM {tbl} "
                                "WHERE COALESCE(session_id, '') <> ''"
                            )
                            row = cur.fetchone()
                            distinct_sessions = max(distinct_sessions, int(row[0] or 0) if row else 0)
                        except Exception:
                            pass
                # Current session turns
                session_id = getattr(getattr(self, "parent", lambda: None)(), "engine", None)
                session_id = getattr(session_id, "session_id", None) or getattr(getattr(self, "window", lambda: None)(), "engine", None)
                session_id = getattr(session_id, "session_id", session_id) if session_id is not None else None
                if isinstance(session_id, str) and session_id:
                    for tbl in ("conversations", "conversation_turns"):
                        if tbl in tables:
                            try:
                                cur.execute(f"SELECT COUNT(*) FROM {tbl} WHERE session_id = ?", (session_id,))
                                row = cur.fetchone()
                                current_session_turns = max(current_session_turns, int(row[0] or 0) if row else 0)
                            except Exception:
                                pass
                con.close()

            # Database size (use memory_db for display, or fallback to user_db)
            db_path = mem_db or user_db
            size_mb = Path(db_path).stat().st_size / 1048576 if db_path and Path(db_path).exists() else 0.0

            daemon_running = bool(getattr(self, "daemon", None))

            health_lines = [
                f"💾 Memory Entries      : {mem_entries}",
                f"💬 Conversation Turns  : {conv_total}",
                f"🧵 Current Session     : {current_session_turns}",
                f"🗂 Distinct Sessions   : {distinct_sessions}",
                f"📊 Database Size       : {size_mb:.2f} MB",
                f"📋 Tables              : {', '.join(tables[:6]) if tables else 'N/A'}",
                f"🗄 DB File             : {Path(db_path).name if db_path else 'unknown'}",
                "",
                f"{'✅' if daemon_running else '⚠'} Proactive Daemon    : {'Running' if daemon_running else 'Not attached'}",
...
class SelfImproveDockWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.recent_text.setPlainText("No improvements analysed yet.")
    ...
    def run_analysis(self):
        ...
        try:
            ...
            if not self.improvements:
                self.issues_text.setPlainText("(0 issues returned by current analyzer run)")
                self.recent_text.setPlainText("(0 improvement records returned)")
                return
            issue_lines = []
            recent_lines = []
            for imp in self.improvements[:12]:
                fname = imp.get('file', 'unknown')
                sug = imp.get('suggestion', '')
                issue_lines.append(f"⚠ {fname}\n   {sug}\n")
                recent_lines.append(f"• {fname}: {sug}")
            self.issues_text.setPlainText('\n'.join(issue_lines))
            self.recent_text.setPlainText('\n'.join(recent_lines[:6]))
        except Exception as e:
            self.issues_text.setPlainText(f"Analysis error: {e}")
            self.recent_text.setPlainText(f"Self-improvement analysis unavailable: {e}")
    ...
    def open_vscode(self):
        ...
        try:
            ...
            subprocess.Popen(["code",
                os.path.join(root, "tools", "automation", "executor_enhanced.py"),
                os.path.join(root, "tools", "automation", "router_enhanced.py")])
        except Exception as e:
...
class IDEDockWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        _paths = get_paths()
        self.projects_dir = Path(os.environ.get("ELI_PROJECTS_DIR", str(_paths.data_dir / "projects")))
        self.scripts_dir = Path(os.environ.get("ELI_SCRIPTS_DIR", str(_paths.data_dir / "scripts")))
...
class ELIWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Non‑CHAT path
            from eli.tools.automation.executor_enhanced import execute as _execute
...
class ELIEntropicGUI(QMainWindow):
    ...
    # FIX: _flush_stream_buffer moved before __init__ so it exists when timer connects
    ...
    def __init__(self):
        ...
        # Autocorrect state
        self.autocorrect_enabled = False
        self._spell = None
        try:
            if "Speller" in globals() and callable(Speller):
                self._spell = Speller()
        ...
        # Streaming buffer – timer connected to the already defined _flush_stream_buffer
        self._stream_buf = []
        ...
        # Initialize TTS state early so async callbacks cannot crash speak()
        self.tts_enabled = getattr(self, "tts_enabled", False)
        self.tts_engine = getattr(self, "tts_engine", None)
        self.tts_voice = getattr(self, "tts_voice", None)
        self.tts_busy = getattr(self, "tts_busy", False)

        # Initialize cognitive engine and check GGUF
        self.engine = get_engine()
        # ----- DEBUG: Verify GGUF model availability -----
        try:
            from eli.brain.cognition import gguf_inference  # correct path
            model_path = gguf_inference.get_model_path()
            gguf_loaded = gguf_inference.load_model() is not None
            print(f"[DEBUG] GGUF model path: {model_path}")
            print(f"[DEBUG] GGUF loaded: {gguf_loaded}")
        except Exception as _gguf_dbg_e:
            print(f"[DEBUG] GGUF check failed: {_gguf_dbg_e}")
        # ------------------------------------------------
    ...
    def _fallback_volume(self, kind: str, payload: dict):
        """Volume control — pactl primary (bypasses WirePlumber restore), wpctl/amixer fallback."""
        import subprocess
        ...
        try:
            ...
            if kind == "VOLUME_SET":
                ...
                # pactl first — bypasses WirePlumber normalization that reverts volume
                ...
                subprocess.run(["wpctl", "set-volume", "@DEFAULT_AUDIO_SINK@", f"{level}%"],
                               check=True, timeout=1.5)
                return _ok(f"Volume set to {level}%")

            if kind == "VOLUME_UP":
                step = int(payload.get("step", 5))
                r = subprocess.run(["pactl", "set-sink-volume", "@DEFAULT_SINK@", f"+{step}%"],
                                   capture_output=True, timeout=1.5)
                if r.returncode != 0:
                    subprocess.run(["wpctl", "set-volume", "-l", "1.0", "@DEFAULT_AUDIO_SINK@", f"{step}%+"],
                                   check=True, timeout=1.5)
                pct = self._linux_get_volume_percent()
            ...
            if kind == "VOLUME_MUTE":
                r = subprocess.run(["pactl", "set-sink-mute", "@DEFAULT_SINK@", "1"],
                                   capture_output=True, timeout=1.5)
                if r.returncode != 0:
            ...
            if kind == "VOLUME_UNMUTE":
                r = subprocess.run(["pactl", "set-sink-mute", "@DEFAULT_SINK@", "0"],
                                   capture_output=True, timeout=1.5)
                if r.returncode != 0:
            ...
            try:
                ...
                if kind == "VOLUME_UP":
                    ...
                    return _ok(f"Volume raised by {step}%")
                if kind == "VOLUME_DOWN":
                    step = int(payload.get("step", 5))
                    subprocess.run(["amixer", "set", "Master", f"{step}%-"], check=True, timeout=1.5)
                    return _ok(f"Volume lowered by {step}%")
                if kind == "VOLUME_MUTE":
    ...
    def _dispatch_local_command(self, kind: str, payload: dict):
        ...
        try:
            if kind == "MEDIA_NEXT":
                ...
                # playerctl play-pause is a single atomic toggle — use it directly
                import subprocess, shutil
                if shutil.which("playerctl"):
                    import subprocess as _sp
                    # Get active player first
                    pl_list = _sp.run(["playerctl", "--list-all"], capture_output=True, text=True, timeout=3)
                    player = None
                    if pl_list.returncode == 0 and pl_list.stdout.strip():
                        players = [p.strip() for p in pl_list.stdout.splitlines() if p.strip()]
                        for pref in ("spotify", "vlc", "mpv", "firefox", "chromium"):
                            for p in players:
                        ...
                        if not player and players:
                            player = players[0]
                    if player:
                        r = _sp.run(["playerctl", "-p", player, "play-pause"], capture_output=True, text=True, timeout=3)
                        if r.returncode == 0:
                            return {"ok": True, "content": f"⏯ Toggled — {player}", "response": f"⏯ Toggled — {player}"}
                        return {"ok": False, "content": f"Toggle failed: {r.stderr.strip()}", "response": "Toggle failed"}
                    return {"ok": False, "content": "No media player running", "response": "No player running"}
                # Fall back to executor PAUSE_MEDIA
    ...
    def init_left_panel(self, main_layout):
        ...
        # TTS speak button
        self.tts_button = QPushButton("🔊 Speak Last")
        self.tts_button.setObjectName("ttsButton")
        self.tts_button.setToolTip("Speak the last ELI response")
        self.tts_button.setStyleSheet(
            "QPushButton { background: #3b4252; color: #88c0d0; border: 1px solid #4c566a; "
            "border-radius: 4px; padding: 4px; font-size: 11px; } "
            "QPushButton:hover { background: #4c566a; }"
        )
        self.tts_button.clicked.connect(self._speak_last_response)
        audio_layout.addWidget(self.tts_button)

        # Auto-speak toggle
        self.tts_auto_button = QPushButton("🔇 Auto-Speak: OFF")
        self.tts_auto_button.setCheckable(True)
        self.tts_auto_button.setObjectName("ttsAutoButton")
        self.tts_auto_button.setToolTip("Auto-speak every ELI response")
        self.tts_auto_button.setStyleSheet(
            "QPushButton { background: #3b4252; color: #88c0d0; border: 1px solid #4c566a; "
            "border-radius: 4px; padding: 4px; font-size: 11px; } "
            "QPushButton:checked { background: #2e3440; color: #a3be8c; border-color: #a3be8c; } "
            "QPushButton:hover { background: #4c566a; }"
        )
        self.tts_auto_button.toggled.connect(self._toggle_auto_speak)
        audio_layout.addWidget(self.tts_auto_button)
    ...
    def init_right_panel(self, main_layout):
        ...
        if not hasattr(self, 'reasoning_mode_combo'):
            self.init_reasoning_toolbar()
    ...
    def _init_tts(self):
        try:
            ...
            for voice in self.tts_engine.getProperty("voices"):
                ...
                if "am-us" in vid or "en_us" in vid or "us" in vid:
    ...
    def speak(self, text: str):
        if not getattr(self, 'tts_enabled', False) or not getattr(self, 'tts_engine', None) or not text:
            return
        try:
    ...
    def apply_autocorrect(self, text: str) -> str:
        if not self.autocorrect_enabled or not self._spell or not text:
            return text
        try:
    ...
    def init_reasoning_toolbar(self):
        toolbar = QToolBar("Reasoning", self)
        toolbar.setMovable(False)
        self.addToolBar(toolbar)
        toolbar.addWidget(QLabel("Reasoning:"))
        self.reasoning_mode_combo = QComboBox(self)
        self.reasoning_mode_combo.addItems(["Quick", "Chain of Thought", "Self-Consistency", "Tree of Thoughts", "Constitutional AI"])
        self.reasoning_mode_combo.currentTextChanged.connect(self.change_reasoning_mode)
        toolbar.addWidget(self.reasoning_mode_combo)

    def change_reasoning_mode(self, label: str):
        mapping = {
            "Quick": "quick",
        ...
        mode = mapping.get(label, "quick")
        if hasattr(self, "engine") and hasattr(self.engine, "set_reasoning_mode"):
            self.engine.set_reasoning_mode(mode)
        self.add_system_message("SYSTEM", f"Reasoning mode changed to {label}", "#88c0d0")

    def _speak_response(self, text: str):
        """Speak via piper TTS in a background thread."""
        ...
        if printable / max(len(text), 1) < 0.8:
            print("[TTS] Skipping binary/non-text content")
            return
        # Strip markdown
        ...
        def _run():
            try:
                ...
                if proc.returncode == 0 and proc.stdout:
                    ...
                    subprocess.run(["espeak-ng", "-s", "165", clean],
                                   capture_output=True, timeout=30)
            except Exception as e:
                print(f"[TTS] error: {e}")
    ...
    def _speak_last_response(self):
        """Speak the last ELI response stored in self._last_eli_response."""
        text = getattr(self, '_last_eli_response', '')
        if text:
            self._speak_response(text)
        else:
            print("[TTS] No ELI response to speak yet")

    def _toggle_auto_speak(self, checked: bool):
        """Toggle automatic TTS for every ELI response."""
        self._tts_auto = checked
        if checked:
            self.tts_auto_button.setText("🔊 Auto-Speak: ON")
        else:
            self.tts_auto_button.setText("🔇 Auto-Speak: OFF")
        print(f"[TTS] Auto-speak: {'ON' if checked else 'OFF'}")
    ...
    def update_system_info(self):
        ...
        try:
            import psutil
            proc = psutil.Process()
            mem_used = int(proc.memory_info().rss / 1048576)
            mem_total = int(psutil.virtual_memory().total / 1048576)
        except Exception:
            mem_used = 0
            mem_total = 0
        self.memory_display.setText(f"Memory: {mem_used}/{mem_total} MB" if mem_total else "Memory: unavailable")
    ...
    def toggle_audio_input(self, checked):
        if checked:
            ...
            # Prevents duplicate callback registration if toggled off/on rapidly.
            ...
            self._audio_callback = audio_callback  # store reference to prevent GC + re-reg guard
            start_audio_listening(self._audio_callback)
    ...
    def process_input_stream(self):
        ...
        user_input = self.apply_autocorrect(user_input)
        if not user_input:
            return
        ...
        if user_input.strip().lower() in PRIORITY_COMMANDS:
            from eli.tools.automation.router_enhanced import route as _route
            from eli.tools.automation.executor_enhanced import execute as _execute
        ...
        if getattr(self, "_busy", False):
            self.add_system_message("SYSTEM", f"Busy: {getattr(self, 'active_workers', 1)} worker(s) still running.", "#ebcb8b")
            return
        ...
        try:
            ...
            if action == "CHAT":
                ...
                return
            # non-chat command — run in ELIWorker to keep GUI responsive
            # Show progress for long-running tasks
            if action in ("GENERATE_SCRIPT", "FIX_FILE", "ANALYZE_PDF"):
                self.process_indicator.setText(f"◉ {action.replace("_", " ")} (background...)")
                self.add_system_message("SYSTEM", f"⏳ Running {action.replace("_", " ").title()} in background — GUI remains responsive.", "#88c0d0")
            self._eli_worker = ELIWorker(action, args, user_input)
            self._eli_worker.execution_complete.connect(self.handle_execution_result)
            self._eli_worker.system_error.connect(lambda e: (
                self.add_system_message("ERROR", f"Command failed: {e}", "#bf616a"),
                self.set_system_status("ERROR"),
                setattr(self, "_busy", False),
                self.process_indicator.setText("◌ IDLE"),
                self.execute_button.setEnabled(True)
            ))
            self._eli_worker.finished.connect(lambda: (
                setattr(self, "_busy", False),
                self.process_indicator.setText("◌ IDLE"),
                self.execute_button.setEnabled(True),
                self.set_system_status("OPERATIONAL")
            ))
            self._eli_worker.start()
            return
        except Exception as e:
