...
def recommend_optimal_settings(sysinfo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Project-wide baseline is ctx=16384 and max_tokens=2048 unless the machine
    ...
    max_tokens = 2048 if n_ctx >= 8192 else 1024
...
class LocalModelManager:
    ...
    def load_model(self, model_path: str, n_ctx: int = 16384,
...
class _GlobalScrollZoom(QObject):
    """Intercepts Ctrl+Wheel on any scrollable/text widget and zooms its font."""
        ...
        try:
            if event.type() == QEvent.Type.Wheel:
                ...
                if mods & ctrl:
                    ...
                    # QTextEdit / QPlainTextEdit — built-in zoom
                    if hasattr(obj, 'zoomIn') and hasattr(obj, 'zoomOut'):
                        if step > 0:
                            obj.zoomIn(1)
                        else:
                            obj.zoomOut(1)
                        event.accept()
                        return True
                    # Everything else — adjust font size on the widget
                    if hasattr(obj, 'font') and hasattr(obj, 'setFont'):
                        f = obj.font()
                        new_size = max(6, f.pointSize() + step)
...
class EliMainWindow(QMainWindow):
    ...
    def _on_stt_transcript(self, text: str):
        ...
        print(f"[STT→GUI] {text}", flush=True)

        # Existing native command path first.
        # No voice_command_safety, no prefix guard, no debounce shim.
        try:
            route = executor_bridge.route_command(text)
            action = str((route or {}).get("action") or "").upper()
            args = (route or {}).get("args") or {}

            # Only bypass CognitiveEngine for real executable actions.
            if action and action != "CHAT":
                try:
                    self.chat_display.append(
                        f'\n<b><span style="color:#a3be8c;">🧑 You [{now_hms()}]:</span></b>'
                        f'<br>{text}<br>'
                    )
                except Exception:
                    pass
                ...
                try:
                    ...
                    print(f"[STT_NATIVE_EXEC_RESULT] {result}", flush=True)

                return

        except Exception as e:
            print(f"[STT_NATIVE_ROUTER_ERROR] {type(e).__name__}: {e}", flush=True)
    ...
    def create_chat_tab(self):
        ...
        self.chat_input.setPlaceholderText(
            "Type your message here… or drag & drop files/images  (Ctrl+Return to send)"
        )
            ...
            try:
                ...
                if _ce is not None:
                    try:
                        ...
                        reply = str(reply) if not isinstance(reply, str) else reply
    ...
    _QA_CATEGORIES = {
        ...
        "Memory":  {"MEMORY_RECALL", "MEMORY_STORE", "MEMORY_STATS",
                    "CLEAR_CHAT_HISTORY"},
        "AI":      {"SELF_ANALYZE", "SELF_IMPROVE", "SELF_PATCH", "SELF_TEST", "SELF_UPGRADE",
    ...
    def _engine_ask(self, prompt: str, max_tokens: int = 512) -> str:
        """Synchronous ELI inference for Labs sub-tabs. Blocks the calling thread."""
        if not prompt or not prompt.strip():
            return "(empty prompt)"
        # Clamp max_tokens — very small values cause llama.cpp segfaults
        max_tokens = max(256, min(max_tokens, 2048))
        try:
            _ce = getattr(self, '_cognitive_engine', None)
            if _ce is not None and hasattr(_ce, 'process'):
                result = _ce.process(prompt, stream=False, reasoning_mode='quick')
                if isinstance(result, str):
                    return result
                import types as _t
                if isinstance(result, _t.GeneratorType) or hasattr(result, '__next__'):
        ...
        try:
            ...
            if backend is not None and hasattr(backend, 'chat'):
                ...
                resp = backend.chat(messages=msgs, max_tokens=max_tokens, temperature=0.7)
...
'''
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.max_tokens_input.setValue(int(s.get("max_tokens", 2048)))
    ...
    def save_settings(self, silent: bool = False):
        ...
        # max_tokens is a per-inference param — not a model-load param. Exclude it
        # from the reload check so that -1 (unlimited, set by launcher) vs a
        # spinbox-clipped value doesn't spuriously unload the model on startup.
        reload_keys = {
        ...
        # Only unload on explicit user saves (silent=False). Silent saves come from
        # the color picker and first-run wiring — they shouldn't trigger a reload.
        try:
