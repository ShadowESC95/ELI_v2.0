...
try:
    ...
    QT_VERSION = 6
    # Define constants for Qt6
    _QT_MATCH_STARTS_WITH = Qt.MatchFlag.MatchStartsWith
    _QT_KEY_ENTER = Qt.Key.Key_Enter
    _QT_KEY_RETURN = Qt.Key.Key_Return
    _QT_KEY_ESCAPE = Qt.Key.Key_Escape
    _QT_KEY_UP = Qt.Key.Key_Up
    _QT_KEY_DOWN = Qt.Key.Key_Down
    _QT_KEY_TAB = Qt.Key.Key_Tab
    _QT_KEY_SPACE = Qt.Key.Key_Space
    _QT_CTRL_MOD = Qt.KeyboardModifier.ControlModifier
    _QT_EVENT_KEYPRESS = QEvent.Type.KeyPress
    print("✓ Using PyQt6")
except ImportError:
    try:
        ...
        QT_VERSION = 5
        # Define constants for Qt5
        _QT_MATCH_STARTS_WITH = Qt.MatchStartsWith
        _QT_KEY_ENTER = Qt.Key_Enter
        _QT_KEY_RETURN = Qt.Key_Return
        _QT_KEY_ESCAPE = Qt.Key_Escape
        _QT_KEY_UP = Qt.Key_Up
        _QT_KEY_DOWN = Qt.Key_Down
        _QT_KEY_TAB = Qt.Key_Tab
        _QT_KEY_SPACE = Qt.Key_Space
        _QT_CTRL_MOD = Qt.ControlModifier
        _QT_EVENT_KEYPRESS = QEvent.KeyPress
        print("✓ Using PyQt5")
    except ImportError:
...
# Import ELI tools
try:
    # First try to import MKII executor (with streaming)
    try:
        ...
        print("✓ Imported MKII executor with streaming support")
        STREAMING_AVAILABLE = True
    except ImportError as e:
        print(f"MKII executor not available: {e}")
        ...
        try:
            ...
            print("✓ Imported regular executor (no streaming)")
        except ImportError as e:
            print(f"Regular executor not available: {e}")
            raise
    
    from eli_tools.capability_registry import list_capabilities, render_detailed
    ELI_AVAILABLE = True
    print("✓ ELI tools available")
except ImportError as e:
...
class ChatStreamWorker(QObject):
    ...
    debug = pyqtSignal(str)  # For debugging
    ...
    def run(self):
        try:
            self.debug.emit(f"Starting chat with message: {self.message[:50]}...")
            
            # Check if streaming is available
            if STREAMING_AVAILABLE and chat_stream is not None:
                self.debug.emit("Using streaming mode")
                buf = []
                try:
                    # Call chat_stream and iterate through tokens
                    stream_gen = chat_stream(self.message)
                    self.debug.emit(f"Got stream generator: {stream_gen}")
                    ...
                    for tok in stream_gen:
                        ...
                        if tok:
                            # Ensure we're emitting strings
                            token_str = str(tok).strip()
                            if token_str:
                                buf.append(token_str)
                                self.token.emit(token_str)
                                self.debug.emit(f"Emitted token: {token_str[:20]}...")
                    
                    content = "".join(buf)
                    self.debug.emit(f"Stream completed. Total content length: {len(content)}")
                    self.done.emit({"ok": True, "content": content, "response": content})
                    
                except Exception as e:
                    self.error.emit(f"Stream error: {str(e)}")
                    self.debug.emit(f"Stream exception details: {type(e).__name__}: {e}")
                    # Try regular chat as fallback
                    self._try_regular_chat()
                    
            else:
                self.debug.emit("Streaming not available, using regular chat")
                self._try_regular_chat()
                
        except Exception as e:
            self.error.emit(f"Worker error: {str(e)}")
            self.debug.emit(f"Worker exception: {type(e).__name__}: {e}")
            self.done.emit({"ok": False, "error": str(e), "content": f"Error: {e}"})

    def _try_regular_chat(self):
        """Try regular chat if streaming fails or isn't available."""
        try:
            self.debug.emit("Calling regular chat() function")
            res = chat(self.message)
            self.debug.emit(f"chat() returned: {type(res)}")
            
            if isinstance(res, dict):
                content = res.get("content", "") or res.get("response", "")
                self.debug.emit(f"Got content length: {len(content)}")
                
                if content:
                    # If we got content all at once, simulate immediate streaming
                    # by sending it as one chunk right away
                    self.token.emit(content)
                    self.done.emit(res)
                else:
                    self.done.emit({"ok": False, "error": "no_content", "content": "No response content"})
            else:
                self.debug.emit(f"Unexpected response type: {type(res)}")
                self.done.emit({"ok": False, "error": "bad_result", "content": str(res)})
                
        except Exception as e:
            self.debug.emit(f"Regular chat error: {type(e).__name__}: {e}")
            self.done.emit({"ok": False, "error": str(e), "content": f"Error in chat: {e}"})
...
class EliProAudioGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        self._last_stream_time = 0  # Track when we last received a token
    ...
    def _send_chat_message(self):
        ...
        self._log_activity(f"Streaming: {STREAMING_AVAILABLE}")
        ...
        self._stream_worker.debug.connect(self._on_stream_debug)
    ...
    def _on_stream_token(self, token):
        ...
        if self._streaming_index is not None and self._streaming_index < len(self._chat_items):
            ...
            # Force immediate render for streaming
            self._render_chat_now()
            
    def _render_chat_now(self):
        """Render chat immediately without debouncing."""
        self._render_pending = False
    ...
    def _on_stream_error(self, error_msg):
        """Handle stream error."""
        self._log_activity(f"Stream error: {error_msg}")
        
    @pyqtSlot(str)
    def _on_stream_debug(self, debug_msg):
        """Handle debug message."""
        self._log_activity(f"Debug: {debug_msg}")
        if self._debug_text:
            self._debug_text.append(f"[{datetime.now().strftime('%H:%M:%S')}] {debug_msg}")
    ...
    def _create_debug_window(self):
        """Create debug window."""
        self._debug_window = QWidget()
        self._debug_window.setWindowTitle("ELI Debug")
        self._debug_window.setGeometry(400, 200, 600, 400)
