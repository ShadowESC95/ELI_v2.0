...
from core.first_run_wizard import run_bootstrap
run_bootstrap()
...
def _eli_gui_longform_guard(message: str, intent: dict) -> dict:
    ...
    strong_prefixes = (
        ...
        "generate document", "create document", "write a document", "write document",
        "generate code", "write code", "create code", "generate script", "write a script",
        "generate project", "create project", "build project",
    )
    grounded_actions = {
        ...
        "LIST_CAPABILITIES", "AWARENESS_STATUS", "CODE_CHANGES",
    }
    ...
    if action in grounded_actions:
        return intent
    if meta.get("need_grounding") or meta.get("allow_chat_without_evidence") is False:
        return intent
    if str(meta.get("task_family") or "").startswith("grounded_"):
        return intent
...
def _choose_default_model(paths_obj=None):
    candidates = []

    if paths_obj is not None:
        try:
            model_candidate = _coerce_pathlike(getattr(paths_obj, "model", None))
            if model_candidate:
                candidates.append(model_candidate)
        except Exception:
            pass
        try:
            models_dir_candidate = _coerce_pathlike(getattr(paths_obj, "models_dir", None))
            if models_dir_candidate and models_dir_candidate.exists():
                for p in sorted(models_dir_candidate.glob("*.gguf")):
                    candidates.append(_coerce_pathlike(p))
        except Exception:
            pass

    env_model = _coerce_pathlike(os.environ.get("ELI_GGUF_MODEL_PATH"))
    if env_model:
        candidates.append(env_model)

    env_models_dir = _coerce_pathlike(os.environ.get("ELI_MODELS_DIR"))
    if env_models_dir and env_models_dir.exists():
        for p in sorted(env_models_dir.glob("*.gguf")):
            candidates.append(_coerce_pathlike(p))

    repo_models_dir = (Path(__file__).resolve().parents[1] / "models").resolve()
    if repo_models_dir.exists():
        for p in sorted(repo_models_dir.glob("*.gguf")):
            candidates.append(_coerce_pathlike(p))
    ...
    if existing:
        preferred_names = [
            ...
            "eli-finetuned-phi3.Q8_0.gguf",
        ]
        for preferred in preferred_names:
            for cand in existing:
                if cand.name == preferred:
                    return cand, cand.parent
        return existing[0], existing[0].parent

    fallback_dir = env_models_dir or repo_models_dir
    fallback_dir = _coerce_pathlike(fallback_dir) or repo_models_dir
    fallback_model = fallback_dir / "Qwen2.5-32B-Instruct-Q4_K_M.gguf"
    return fallback_model, fallback_dir

if CENTRAL_IMPORTS_AVAILABLE and get_paths:
    _paths = get_paths()
    PROJECT_ROOT = _coerce_pathlike(getattr(_paths, "project_root", None)) or Path(__file__).resolve().parents[1]
    APP_DIR = Path.home() / ".eli_mkvii"
    # SETTINGS_FILE must match runtime_settings._settings_file() → ~/eli/config/settings.json
    try:
        from eli.core import runtime_settings as _rt
        SETTINGS_FILE = _rt._settings_file()
    except Exception:
        SETTINGS_FILE = _coerce_pathlike(getattr(_paths, "config_dir", None) or APP_DIR) / "settings.json"

    MEMORY_DB = _coerce_pathlike(getattr(_paths, "memory_db", None))
    USER_DB = _coerce_pathlike(getattr(_paths, "user_db", None))
    AGENT_DB = _coerce_pathlike(getattr(_paths, "agent_db", None))
    CONVERSATIONS_DIR = _coerce_pathlike(getattr(_paths, "conversations_dir", None)) or (APP_DIR / "conversations")
    ARTIFACTS_DIR = _coerce_pathlike(getattr(_paths, "artifacts_dir", None)) or (APP_DIR / "artifacts")

    _default_model_path_obj, _bundled_model_dir_obj = _choose_default_model(_paths)
    DEFAULT_MODEL_PATH = str(_default_model_path_obj)
    BUNDLED_MODEL_DIR = _bundled_model_dir_obj
    CUSTOM_MODELS_DIR = _coerce_pathlike(getattr(_paths, "models_dir", None)) or APP_DIR / "models"
else:
    ...
    MEMORY_DB = _coerce_pathlike(getattr(PATHS, "memory_db", None))
    USER_DB = _coerce_pathlike(getattr(PATHS, "user_db", None))
    AGENT_DB = _coerce_pathlike(getattr(PATHS, "agent_db", None))
    CONVERSATIONS_DIR = APP_DIR / "conversations"
    ARTIFACTS_DIR = APP_DIR / "artifacts"

    _default_model_path_obj, _bundled_model_dir_obj = _choose_default_model(None)
    DEFAULT_MODEL_PATH = str(_default_model_path_obj)
    BUNDLED_MODEL_DIR = _bundled_model_dir_obj
...
if CENTRAL_IMPORTS_AVAILABLE and config:
    ...
    def _load_eli_persona() -> str:
        import pathlib
        for p in [
            PROJECT_ROOT / "brain" / "persona" / "persona.txt",
            PROJECT_ROOT / "brain" / "cognition" / "persona.txt",
        ]:
            if p.exists():
                return p.read_text(encoding="utf-8", errors="replace").strip()
        return "You are ELI, a local assistant."
...
def _fmt_ts(ts) -> str:
    """Format a raw unix float or string timestamp into readable datetime."""
    if not ts:
        return "unknown"
    try:
...
def recommend_optimal_settings(sysinfo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    n_gpu_layers = 14 if has_gpu else 0  # RTX 2060 SUPER: 14 layers optimal
    n_threads = cpu_count
    temperature = 0.7
    max_tokens = 512
...
def _normalize_assistant_text(user_text: str, model_text: str) -> str:
    text = _sanitize_identity_drift(model_text or "")
    text = _policy_identity_memory_response(user_text or "", text)
    text = re.sub(r"^\s*ELI\s*:\s*", "", text, flags=re.IGNORECASE)
    text = text.replace(" ", " ")
    return text.strip()
...
def _provider_label(key: str) -> str:
    return MODEL_PROVIDER_LABELS.get(key, key)



class _NullMemorySystem:
    ...
    def get_habit_rules(self, *args, **kwargs):
        return []

    def add_habit_rule(self, *args, **kwargs):
        return -1

    def delete_habit_rule(self, *args, **kwargs):
        return None

    def toggle_habit_rule(self, *args, **kwargs):
        return None


memory_system = _NullMemorySystem()
if CENTRAL_IMPORTS_AVAILABLE and get_memory:
    try:
        memory_system = CentralMemoryAdapter(get_memory())
    except Exception as e:
        print(f"[GUI] memory_system init failed: {e}")
...
class LocalModelManager:
    ...
    def load_model(self, model_path: str = "", n_ctx: int = 8192,
                   n_threads: int = 8, n_gpu_layers: int = 0, **kwargs) -> bool:
        try:
            from eli.core import config
            from eli.brain.cognition import gguf_inference
            ...
            if model_path:
                ...
                config.set_gguf_model_path(str(path_obj))
            else:
                path_obj = resolve_model_path(config.get_gguf_model_path() or "")
                self.model_path = str(path_obj) if path_obj else ""

            if hasattr(config, "set_gguf_n_ctx"):
                config.set_gguf_n_ctx(int(n_ctx))
            if hasattr(config, "set_n_threads"):
                config.set_n_threads(int(n_threads))
            else:
                config.set("gguf_n_threads", int(n_threads))
            if hasattr(config, "set_gguf_n_gpu_layers"):
                config.set_gguf_n_gpu_layers(int(n_gpu_layers))
            if "n_batch" in kwargs and hasattr(config, "set_gguf_n_batch"):
                config.set_gguf_n_batch(int(kwargs["n_batch"]))
            elif "batch_size" in kwargs and hasattr(config, "set_gguf_n_batch"):
                config.set_gguf_n_batch(int(kwargs["batch_size"]))
            ...
            self.load_error = None if self.is_loaded else "GGUF singleton failed to load"
        ...
        try:
            ...
            for m in messages:
                ...
                if role == "system":
                    ...
                    prompt_parts.append(f"Assistant said previously: {content}")
            prompt = "\n".join(prompt_parts).strip()
            from eli.brain.cognition import gguf_inference
            return gguf_inference.chat_completion(
                prompt=prompt,
                system=system or ELI_SYSTEM_PROMPT,
                max_tokens=max_tokens,
                temperature=temperature,
            )
        except Exception as e:
    ...
    def chat_stream(self, messages, max_tokens=1024, temperature=0.7):
        ...
        try:
            ...
            prompt = "\n".join(prompt_parts).strip()
            from eli.brain.cognition import gguf_inference
            for chunk in gguf_inference.generate(
                prompt=prompt,
                system=system or ELI_SYSTEM_PROMPT,
                max_tokens=max_tokens,
                temperature=temperature,
...
# ── Quick action fast path for voice commands (added by patch) ──
_ELI_QUICK_VOICE_ACTIONS = {
    'OPEN_APP', 'CLOSE_APP', 'PLAY_MEDIA', 'PAUSE_MEDIA', 'STOP_MEDIA',
    'NEXT_MEDIA', 'PREVIOUS_MEDIA', 'VOLUME', 'MUTE', 'UNMUTE',
    'TIME', 'DATE', 'SET_TIMER', 'SET_ALARM',
    'OPEN_BROWSER', 'OPEN_IDE', 'OPEN_SYSTEM_SETTINGS',
    'OPEN_AUDIO_SETTINGS', 'OPEN_POWER_SETTINGS',
    'SCREENSHOT', 'SET_CLIPBOARD', 'GET_CLIPBOARD',
}

def _eli_is_quick_voice_action(text: str) -> tuple:
    """Check if text routes to a quick action. Returns (is_quick, intent)."""
    try:
        from tools.automation.router_enhanced import route
        intent = route(text)
        action = str((intent or {}).get('action') or '').upper()
        conf = float((intent or {}).get('confidence') or 0)
        if action in _ELI_QUICK_VOICE_ACTIONS and conf >= 0.8:
            return True, intent
    except Exception:
        pass
    try:
...
def _eli_should_bypass_cognitive_engine(intent: Dict[str, Any]) -> bool:
    action = str((intent or {}).get('action') or '').upper()
    direct_actions = {
        'DOC_GENERATE', 'GENERATE_DOCUMENT', 'CREATE_DOCUMENT',
        'GENERATE_SCRIPT', 'GENERATE_PROJECT',
        'WRITE_NOTE', 'FIX_FILE', 'READ_FILE', 'ANALYZE_PDF', 'ANALYZE_CSV',
        'SHOW_DIFF', 'DATA_FABRICATOR'
    }
    return action in direct_actions


def _eli_coerce_executor_payload(result: Any) -> str:
    if isinstance(result, dict):
        # If this is a generated script, flag it for IDE opening
        if result.get('open_in_ide') and result.get('script_path'):
            result['_open_in_ide_path'] = result['script_path']
        return str(result.get('response') or result.get('content') or result.get('error') or result)
    return str(result)
...
class EliMainWindow(QMainWindow):
    ...
    def _auto_open_in_ide(self, result):
        """If executor returned a script, auto-open it in the IDE tab."""
        if not isinstance(result, dict):
            return
        script_path = result.get('_open_in_ide_path') or result.get('script_path')
        if not script_path:
            return
        try:
            from pathlib import Path
            ...
            if p.exists() and p.suffix == '.py':
                ...
                if hasattr(self, 'code_editor'):
                    if hasattr(self.code_editor, 'setText'):
                        self.code_editor.setText(content)
                    else:
                        self.code_editor.setPlainText(content)
                    self.current_file_path = str(p)
                    self.current_file_label.setText(f"File: {p.name}")
                    # Switch to IDE tab (index 5 = IDE)
                    if hasattr(self, 'tabs'):
                        for i in range(self.tabs.count()):
                            if 'IDE' in (self.tabs.tabText(i) or ''):
                                self.tabs.setCurrentIndex(i)
                                break
                    print(f"[GUI] Auto-opened script in IDE: {p.name}")
        except Exception as e:
            print(f"[GUI] Failed to auto-open in IDE: {e}")
    ...
    def __init__(self):
        ...
        self.apply_theme()
        self._apply_user_text_color_preview()
    ...
    def _configure_window_geometry(self):
        """Fit the main window to the current screen while keeping it resizable."""
        try:
            screen = None
            try:
            ...
            if screen is not None:
                ...
                avail_w = max(int(available.width()), 820)
                avail_h = max(int(available.height()), 560)

                target_w = min(max(int(avail_w * 0.94), 820), avail_w)
                target_h = min(max(int(avail_h * 0.90), 560), avail_h)
            ...
            print(f"[GUI] Window geometry auto-fit failed: {e}")
    ...
    def _speak_response(self, text: str):
        ...
        def _run():
            try:
                piper_bin = os.environ.get('ELI_PIPER_BIN') or shutil.which('piper')
                voice_candidates = []
                env_model = os.environ.get('ELI_TTS_MODEL', '').strip()
                if env_model:
                    voice_candidates.append(Path(env_model).expanduser())
                voice_candidates.extend([
                    PROJECT_ROOT / 'voices' / 'en_US-amy-medium.onnx',
                    APP_DIR / 'voices' / 'en_US-amy-medium.onnx',
                ])
                model_path = next((str(c) for c in voice_candidates if c.exists()), None)
                if piper_bin and model_path:
                    p = subprocess.run([piper_bin, '--model', model_path, '--output-raw'],
    ...
    def _on_stt_transcript(self, text: str):
        ...
        # Fast path: quick actions bypass the LLM and is_generating lock
        is_quick, intent = _eli_is_quick_voice_action(text)
        if is_quick and intent:
            action = str(intent.get('action') or '').upper()
            args = dict(intent.get('args') or {})
            print(f"[STT→GUI] ⚡ Quick action: {action}")
            def _quick_worker():
                try:
                    from brain.cognition.cognitive_engine import get_engine
                    engine = get_engine()
                    result = engine.process(text, stream=False, reasoning_mode="quick")
                    if isinstance(result, dict):
                        response = result.get('content') or result.get('response') or ''
                    else:
                    ...
                    print(f"[STT→GUI] Quick action error: {e}")
                    # Fallback: try executor directly
                    try:
                        from tools.automation.executor_enhanced import execute
                        result = execute(action, args)
                        resp = str(result.get('content') or result.get('response') or result)
                        self.chat_response_signal.emit("__STREAM_START__")
                        self.chat_response_signal.emit(resp)
                        self.chat_response_signal.emit("__STREAM_END__")
                    except Exception:
                        pass
            import threading
            threading.Thread(target=_quick_worker, daemon=True, name="ELI-QuickAction").start()

            # Show command in chat without going through send_message
            display_name = (getattr(self, 'user_name', '') or 'USER*').strip() or 'USER*'
            user_color = self.get_selected_user_text_color() if hasattr(self, 'get_selected_user_text_color') else '#6CB6FF'
            self._append_user_chat_entry(display_name, user_color, text)
            self.conversation_history.append({'role': 'user', 'content': text})
            return

        # Normal path: queue through send_message (respects is_generating)
    ...
    def _si_analyze_failures(self):
        def worker():
            ...
            try:
                ...
                for row in rows:
                    ts = _fmt_ts(row.get("timestamp", "") or row.get("ts", ""))
    ...
    def create_settings_tab(self):
        ...
        self.provider_combo.addItem('Bundled GGUF', 'bundled_gguf')
        self.provider_combo.addItem('Custom GGUF', 'custom_gguf')
        ...
        self.n_gpu_layers_input.setValue(8)
        ...
        self.user_text_color_combo.currentIndexChanged.connect(lambda *_: self._apply_user_text_color_preview())
    ...
    def current_provider(self) -> str:
        import os as _os
        if _os.environ.get("ELI_GGUF_MODEL_PATH", "").strip():
            return "custom_gguf"
        if not hasattr(self, "provider_combo"):
            return "custom_gguf"
        return str(self.provider_combo.currentData() or "custom_gguf")
    ...
    def _apply_user_text_color_preview(self):
        color = self.get_selected_user_text_color()
        try:
            if hasattr(self, 'chat_input'):
                self.chat_input.setStyleSheet(f"color: {color};")
        except Exception:
            pass

    def _append_user_chat_entry(self, display_name: str, user_color: str, user_message: str):
        ts = now_hms()
        ...
        name_fmt.setFontWeight(QFont.Weight.Normal)
        name_fmt.setForeground(QColor(user_color))
        cursor.setCharFormat(name_fmt)
        cursor.insertText(f"🧑 {display_name} [{ts}]:")

        cursor.insertBlock()
        body_fmt = QTextCharFormat()
        body_fmt.setFontWeight(QFont.Weight.Normal)
        body_fmt.setForeground(QColor(user_color))
    ...
    def refresh_model_sources(self):
        ...
        if hasattr(self, 'bundled_model_combo'):
            ...
            for m in models:
                # Show all discovered models regardless of source
    ...
    def resolve_selected_model_path(self) -> str:
        import os as _os
        # Launcher-selected model is ground truth — always use it if set
        env_path = _os.environ.get('ELI_GGUF_MODEL_PATH', '').strip()
        ...
        def load_worker():
            try:
                if provider == 'ollama':
                    ...
                    # Launcher sets ELI_GGUF_MODEL_PATH — use it as ground truth
                    import os as _os
                    _env_path = _os.environ.get('ELI_GGUF_MODEL_PATH', '')
                    _resolved_model_path = (
                        _env_path or
                        getattr(model_manager, 'model_path', None) or
                        model_path or 'model'
                    )
                    try:
                        model_name_display = Path(str(_resolved_model_path)).name
                    except Exception:
    ...
    def send_message(self):
        ...
        display_name = (getattr(self, 'user_name', '') or 'USER*').strip() or 'USER*'
        user_color = self.get_selected_user_text_color() if hasattr(self, 'get_selected_user_text_color') else '#6CB6FF'
        self._append_user_chat_entry(display_name, user_color, user_message)
        ...
        def generate_worker():
            try:
                try:
                    ...
                    intent = _eli_gui_longform_guard(user_message, intent)
                except Exception:
                    intent = {"action": "CHAT", "args": {"message": user_message}, "confidence": 0.0}

                if _eli_should_bypass_cognitive_engine(intent):
                    action = str(intent.get('action') or '').upper()
                    args = dict(intent.get('args') or {})
                    result = None
                    try:
                        if executor_bridge.executor_module and hasattr(executor_bridge.executor_module, 'execute'):
                            result = executor_bridge.executor_module.execute(action, args)
                        else:
                            result = {'ok': False, 'error': 'Executor unavailable for direct action dispatch.'}
                    except Exception as e:
                        result = {'ok': False, 'error': str(e)}

                    response = _eli_coerce_executor_payload(result)
                    self._last_trace = {'direct_action': action, 'intent': intent, 'result_ok': bool(isinstance(result, dict) and result.get('ok'))}
                    self.chat_response_signal.emit("__STREAM_START__")
                    self.chat_response_signal.emit(response)
                    self.chat_response_signal.emit("__STREAM_END__")
                else:
                    try:
                        from brain.cognition.cognitive_engine import get_engine as _get_engine
                    except Exception:
                        _get_engine = None
                    if _get_engine is None:
                        self.chat_response_signal.emit('❌ Central cognitive engine unavailable')
                        self.is_generating = False
                        return
                    engine = _get_engine()
                    result = engine.process(user_message, stream=True, reasoning_mode=getattr(self, "_reasoning_mode", "quick"))
                    ...
                    if hasattr(result, '__iter__') and not isinstance(result, (dict, str)):
                        ...
                        for token in result:
                            ...
                            if first:
                                self.chat_response_signal.emit("__STREAM_START__")
                                first = False
                            self.chat_response_signal.emit(tok)
                        self.chat_response_signal.emit("__STREAM_END__")
                    else:
                        if isinstance(result, dict):
                            response = result.get('content') or result.get('response') or result.get('error') or ''
                            self._last_trace = result.get('trace', {}) or {}
                        else:
                            response = str(result)
                            self._last_trace = {}
                        self.chat_response_signal.emit("__STREAM_START__")
    ...
    def search_memory(self):
        ...
        if results:
            ...
            for i, mem in enumerate(results, 1):
                ts = _fmt_ts(mem.get('timestamp') or mem.get('ts',''))
    ...
    def _update_proactive(self, payload: dict):
        payload = payload or {}
        try:
            suggestions = payload.get('suggestions') or payload.get('html') or ''
            summary = payload.get('summary') or ''
            insights = payload.get('insights') or ''
            if suggestions and hasattr(self, 'suggestions_display'):
                self.suggestions_display.clear()
                self.suggestions_display.append(str(suggestions))
            if summary and hasattr(self, 'summaries_display'):
                self.summaries_display.clear()
                self.summaries_display.append(str(summary))
            if insights and hasattr(self, 'insights_display'):
                self.insights_display.clear()
                self.insights_display.append(str(insights))
        except Exception as e:
            try:
                self._append_log(f'[PROACTIVE] UI update error: {e}')
            except Exception:
                print(f'[PROACTIVE] UI update error: {e}')
    ...
    def save_settings(self, silent: bool = False):
        ...
        if CENTRAL_IMPORTS_AVAILABLE and config:
            try:
                ...
                if gguf_inference:
                    pass  # keep current GGUF model live on settings save
            except Exception as e:
    ...
    def _append_chat_response(self, text: str):
        ...
        if text == "__STREAM_START__":
            ...
            label_fmt.setFontWeight(QFont.Weight.Normal)
        ...
        if text == "__STREAM_END__":
            ...
            final_response = _normalize_assistant_text(getattr(self, "_pending_user_message", ""), "".join(getattr(self, "_stream_buffer", [])))
            if final_response.strip():
                self.conversation_history.append({'role': 'assistant', 'content': final_response})
                self._last_eli_response = final_response
            # Show agent confidence in status bar
            try:
                last_trace = getattr(self, "_last_trace", {}) or {}
                conf = last_trace.get("agent_confidence")
                label = last_trace.get("confidence_label", "")
                agents = last_trace.get("agents_used", [])
                if conf is not None:
                    self.status_signal.emit(
                        f"✅ Ready  |  confidence: {conf:.0%} ({label})  |  agents: {', '.join(agents)}"
                    )
            except Exception:
                pass
        ...
        text = _normalize_assistant_text(getattr(self, "_pending_user_message", ""), text)
        ts = datetime.now().strftime("%H:%M:%S")
        cursor = self.chat_display.textCursor()
        cursor.movePosition(cursor.MoveOperation.End)
        cursor.insertBlock()
