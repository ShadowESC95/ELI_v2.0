...
OUT="$ROOT/ops/reports/phase49_router_residual_public_alias_rebinding_guarded_delete_v2_${STAMP}"
...
cp "$ROUTER" "$OUT/backups/router_enhanced.py.before_phase49_v2.bak"
...
restore_on_failure() {
  ...
  if [[ "$PATCH_APPLIED" == "1" ]]; then
    echo
    echo "PHASE49 v2 FAILURE DETECTED — restoring router from pre-Phase49 backup." >&2
    cp "$OUT/backups/router_enhanced.py.before_phase49_v2.bak" "$ROUTER"
    python3 -m py_compile "$ROUTER" >/dev/null 2>&1 || true
    echo "ROUTER_RESTORED_AFTER_PHASE49_V2_FAILURE" >&2
  fi
  exit "$code"
}
...
# Phase 49 v2 — Residual Public Alias Rebinding Guarded Delete
...
Phase48 left exactly 7 pre-Phase38 public alias rebinding lines:
...
Phase49 v1 removed these successfully, but its final assertion falsely failed
because it used whole-file substring checks rather than AST-scoped,
pre-Phase38 assignment checks.

Phase49 v2 repeats the guarded delete and verifies removal using the same
pre-Phase38 AST boundary logic used by the rewrite itself.
...
8. Targeted residual alias assignments are absent specifically before Phase38.
EOF
...
if marker_line is None:
    raise RuntimeError("Phase38 marker not found during Phase49 v2 rewrite")
...
if missing:
    raise RuntimeError(
        "Phase49 v2 missing expected residual alias assignment(s): "
        + ", ".join(f"{l}={r}" for l, r in sorted(missing))
    )

if len(found) != len(expected):
    raise RuntimeError(
        f"Phase49 v2 expected exactly {len(expected)} matching alias assignments, found {len(found)}"
    )
...
report = [
    "=== PHASE 49 v2 PHASE36 SEMANTIC BASELINE COMPARISON ===",
    f"PRE_JSON={pre_path}",
...
if not raw_equal:
    raise SystemExit("Phase49 v2 failed: raw Phase36 semantic baseline JSON changed")

if not parsed_equal:
    raise SystemExit("Phase49 v2 failed: parsed Phase36 semantic baseline JSON changed")
PY
...
report: list[str] = ["=== PHASE 49 v2 DIRECT RUNTIME PROBE ==="]
failures: list[str] = []
...
if failures:
    raise SystemExit("Phase49 v2 direct runtime probe failed")
PY

# ---------------------------------------------------------------------
# 7. Corrected targeted post-patch assertions
# ---------------------------------------------------------------------
...
if marker_line is None:
    raise RuntimeError("Phase38 marker not found during Phase49 v2 assertion")
...
for node in ast.walk(tree):
    ...
    sig = assignment_signature(node)
    if sig in forbidden_pre_phase38:
        s, e = span(node)
        remaining.append((s, e, sig[0], sig[1]))

assertions: list[str] = []
failures: list[str] = []

if remaining:
    for s, e, left, right in remaining:
        failures.append(
            f"FAIL: forbidden pre-Phase38 alias rebinding remains: "
            f"{left} = {right} lines={s}-{e}"
        )
else:
    assertions.append(
        "PASS: targeted residual public alias assignments are absent before the Phase38 marker"
    )
...
report = [
    "=== PHASE 49 v2 TARGETED POST-PATCH ASSERTIONS ===",
    *assertions,
]
...
if failures:
    raise SystemExit("Phase49 v2 targeted assertions failed")
PY
...
Phase49 v2 succeeded.

Interpretation:
- Phase49 v1's patch operation was sound; its failure was a verification-scope bug.
- Phase49 v2 completes the same repair with a correctly scoped AST-based assertion.
- The two Phase45b-tracked pre-Phase38 debt classes are now closed:
  - route-capture symbol residue: 0
  - public alias rebinding residue: 0

Next target:
- Phase50 should audit the remaining 7 mixed pre-Phase38 Try blocks and classify
  which are:
  1. helper-hosting blocks that should be hoisted into clean standalone helpers;
  2. shell-only blocks that can be deleted;
  3. blocks still dependency-coupled to the Phase38 flattened dispatcher.
