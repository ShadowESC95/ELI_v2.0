...
OUT="$ROOT/ops/reports/phase47b_router_lrf_pm_adapter_chain_guarded_delete_${STAMP}"
...
# ---------------------------------------------------------------------
# 0. Recover from failed Phase47 v1 partial source mutation
# ---------------------------------------------------------------------

FAILED47_OUT="$(ls -td ops/reports/phase47_router_legacy_adapter_chain_guarded_delete_* 2>/dev/null | head -1 || true)"

if [[ -z "$FAILED47_OUT" ]]; then
  echo "Could not locate failed Phase47 report directory for recovery." >&2
  exit 1
fi

FAILED47_BACKUP="$FAILED47_OUT/backups/router_enhanced.py.before_phase47.bak"

if [[ ! -f "$FAILED47_BACKUP" ]]; then
  echo "Missing failed Phase47 backup: $FAILED47_BACKUP" >&2
  exit 1
fi

echo "=== RECOVER FAILED PHASE47 SOURCE ===" | tee "$OUT/00_recovery.txt"
echo "FAILED47_OUT=$FAILED47_OUT" | tee -a "$OUT/00_recovery.txt"
echo "RESTORE_FROM=$FAILED47_BACKUP" | tee -a "$OUT/00_recovery.txt"

cp "$FAILED47_BACKUP" "$ROUTER"

python3 -m py_compile "$ROUTER" 2>&1 | tee -a "$OUT/00_recovery.txt"
echo "RECOVERY_PY_COMPILE_OK" | tee -a "$OUT/00_recovery.txt"

# Keep a fresh backup of the now-restored Phase46-equivalent state.
cp "$ROUTER" "$OUT/backups/router_enhanced.py.before_phase47b.bak"
...
restore_on_failure() {
  ...
  if [[ "$PATCH_APPLIED" == "1" ]]; then
    echo
    echo "PHASE47B FAILURE DETECTED — restoring router from pre-Phase47b backup." >&2
    cp "$OUT/backups/router_enhanced.py.before_phase47b.bak" "$ROUTER"
    python3 -m py_compile "$ROUTER" >/dev/null 2>&1 || true
    echo "ROUTER_RESTORED_AFTER_PHASE47B_FAILURE" >&2
  fi
  exit "$code"
}
...
# Phase 47b — Router LRF / PM Legacy Adapter Chain Guarded Delete

Generated: $(date -Is)  
Root: $ROOT  
Target: $ROUTER

## Correction from failed Phase47 v1

Phase47 v1 removed too much. It deleted the final personal-memory
route/route_intent adapter pair, which is still required by the pre-Phase38
Phase11 multi-PDF wrapper because that wrapper captures:

- \`_eli_phase11_prev_route = route\`
- \`_eli_phase11_prev_route_intent = route_intent\`

Phase47b therefore deletes only the two truly removable legacy adapter chains:

1. legacy_lrf_adapter_chain
2. legacy_pm_adapter_chain

The final personal-memory route adapter pair is explicitly preserved in this pass.

## Expected top-level deletion spans

- LRF chain: 5
- PM chain: 5

Total expected removal spans: 10
EOF
...
if marker_line is None:
    raise RuntimeError("Phase38 marker not found during Phase47b AST prune")
...
def try_rebinds_legacy_route_pair(node: ast.Try, helper_route: str, helper_intent: str) -> bool:
    ...
    for sub in ast.walk(node):
        ...
        if "route" in targets and value_name == helper_route:
            saw_route_or_equivalent = True
        if "route_intent" in targets and value_name == helper_intent:
            saw_intent = True

    # The current source has route/route_intent paired inside these specific
    # adapter-shell Try blocks. Require the full pair to avoid deleting any
    # unrelated single-assignment Try block.
    return saw_route_or_equivalent and saw_intent
...
if not removals:
    raise RuntimeError("Phase47b found no removable LRF/PM legacy adapter chain statements")
...
if len(removals) != EXPECTED_REMOVAL_SPANS:
    raise RuntimeError(
        f"Expected {EXPECTED_REMOVAL_SPANS} Phase47b removal spans, found {len(removals)}:\n"
...
lines = [
    "=== PHASE 47b PHASE36 SEMANTIC BASELINE COMPARISON ===",
    f"PRE_JSON={pre_path}",
...
if not raw_equal:
    raise SystemExit("Phase47b failed: raw Phase36 semantic baseline JSON changed")

if not parsed_equal:
    raise SystemExit("Phase47b failed: parsed Phase36 semantic baseline JSON changed")
PY
...
for chain in (
    ...
    if chain in adapter_inventory:
        assertions.append(f"PASS: retained deferred chain remains classified for later handling: {chain}")
    else:
        failures.append(f"FAIL: expected deferred final-PM adapter chain missing from inventory: {chain}")

report = [
    "=== PHASE 47b TARGETED POST-PATCH ASSERTIONS ===",
    *assertions,
]
if failures:
...
if failures:
    raise SystemExit("Phase47b targeted post-patch assertions failed")
PY

cat > "$OUT/21_console_digest.txt" <<EOF
=== PHASE 47b DIGEST ===
Failed Phase47 v1 source recovery: PASS
Router restored from failed Phase47 backup: PASS
...
Critical correction:
- The final personal-memory route/route_intent adapter pair cannot yet be deleted.
- Phase11 multi-PDF wrapper still captures route_intent before Phase38.
- Phase48 should be a dependency-rewire audit/refactor around Phase11:
  either decouple the multi-PDF wrapper from transient pre-Phase38 route_intent state,
  or move that enrichment into the flattened canonical dispatcher itself.
