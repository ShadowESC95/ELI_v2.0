...
OUT="$ROOT/ops/reports/phase16c_identity_matcher_dataset_portability_repair_${STAMP}"
BACKUP="$OUT/backups"
...
echo "======================================================================"
echo "PHASE 16C — Identity Matcher + Dataset Path Portability Repair"
echo "ROOT : $ROOT"
echo "OUT  : $OUT"
echo "TIME : $(date -Is)"
echo "======================================================================"
...
echo "=== 2. Patch control_contracts.py identity matcher with line-structured replacement ==="
python3 - "$ROOT" "$OUT" <<'PY'
...
marker = "# PHASE16C_IDENTITY_WHAT_ARE_YOU_BOUNDARY_FIX"

if marker in src:
    print("SKIP: Phase 16C identity matcher patch already present.")
...
if target_idx is None:
    ...
    for lineno, line in enumerate(lines, start=1):
        if "what are you" in line:
            nearby.append(f"{lineno}: {line.rstrip()}")
    (out / "02_control_contracts_identity_patch_failure.txt").write_text(
        "Could not find exact old identity matcher line.\n\n"
        + "\n".join(nearby)
        + "\n",
        encoding="utf-8",
    )
    raise SystemExit(
        "PATCH FAILED: old identity matcher line not found. "
...
replacement_lines = [
    f'{indent}# PHASE16C_IDENTITY_WHAT_ARE_YOU_BOUNDARY_FIX\n',
    f'{indent}# Match identity questions such as "what are you?" or\n',
    f'{indent}# "what are you exactly?", but do not match conversational\n',
    f'{indent}# continuations such as "what are you talking about?".\n',
...
(out / "02_control_contracts_identity_patch.txt").write_text(
    "PATCHED control_contracts.py: identity matcher no longer overmatches conversational `what are you ...` continuations.\n",
    encoding="utf-8",
)
...
echo "=== 3. Patch dataset_builder.py: dynamic PROJECT_ROOT redaction instead of historical ELI_MKXI checkout path ==="
python3 - "$ROOT" "$OUT" <<'PY'
from __future__ import annotations
...
marker = "# PHASE16C_DYNAMIC_PROJECT_ROOT_REDACTION"

if marker in src:
    print("SKIP: Phase 16C dataset_builder portability patch already present.")
...
new = '''# PHASE16C_DYNAMIC_PROJECT_ROOT_REDACTION
...
if old not in src:
    ...
    (out / "03_dataset_builder_patch_failure.txt").write_text(
        "Could not find old dataset_builder path-redaction block.\n\n"
        + "\n".join(nearby)
        + "\n",
        encoding="utf-8",
    )
    raise SystemExit(
        "PATCH FAILED: dataset_builder redaction block not found. "
...
(out / "03_dataset_builder_patch.txt").write_text(
    "PATCHED dataset_builder.py: dynamic PROJECT_ROOT redaction and cross-platform HOME path sanitisation installed.\n",
    encoding="utf-8",
)
...
marker = "# PHASE16C_WINDOWS_HOME_REDACTION"

if marker in src:
    print("SKIP: Phase 16C dataset_filters portability patch already present.")
...
if old not in src:
    ...
    (out / "04_dataset_filters_patch_failure.txt").write_text(
        "Could not find dataset_filters home-redaction block.\n\n"
        + "\n".join(nearby)
        + "\n",
        encoding="utf-8",
    )
    raise SystemExit(
        "PATCH FAILED: dataset_filters redaction block not found. "
...
{
  echo "--- Historical ELI_MKXI mention in dataset_builder.py ---"
...
{
  grep -n -C 5 --color=never 'PHASE16C_IDENTITY_WHAT_ARE_YOU_BOUNDARY_FIX' \
    "$ROOT/eli/runtime/control_contracts.py" || true
...

{
  ...
  echo "3. Replaced dataset_builder's historical checkout-specific path redaction with dynamic PROJECT_ROOT redaction."
