...
assert_eq(){
  ...
  if [[ "$actual" == "$expected" ]]; then
    ...
    ((PASS++)); REPORT+="PASS|$label|expected=$expected|actual=$actual\n"
  else
    red  "$label — expected=$expected actual=$actual"
    ((FAIL++)); REPORT+="FAIL|$label|expected=$expected|actual=$actual\n"
  fi
}
...
assert_ge(){
  ...
  if (( actual >= threshold )); then
    ...
    ((PASS++)); REPORT+="PASS|$label|>=$threshold|actual=$actual\n"
  else
    red  "$label — >=$threshold actual=$actual"
    ((FAIL++)); REPORT+="FAIL|$label|>=$threshold|actual=$actual\n"
  fi
}
...
assert_contains(){
  ...
  if echo "$haystack" | grep -qF "$needle"; then
    ...
    ((PASS++)); REPORT+="PASS|$label|contains='$needle'\n"
  else
    red  "$label — missing '$needle'"
    ((FAIL++)); REPORT+="FAIL|$label|missing='$needle'\n"
  fi
}
...
assert_file_exists(){
  ...
  if [[ -f "$f" ]]; then
    ...
    ((PASS++)); REPORT+="PASS|$label|file=$f\n"
  else
    red  "$label — $f MISSING"
    ((FAIL++)); REPORT+="FAIL|$label|file=$f\n"
  fi
}
...
check_file_lines(){
  ...
  if (( actual >= expected_min )); then
    ...
    ((PASS++)); REPORT+="PASS|$label|lines>=$expected_min|actual=$actual\n"
  else
    red  "$label — lines>=$expected_min actual=$actual"
    ((FAIL++)); REPORT+="FAIL|$label|lines>=$expected_min|actual=$actual\n"
  fi
}
