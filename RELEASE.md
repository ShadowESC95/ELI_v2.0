...
# Output: dist/app_packages/ELI_v2-2.0.14-linux-portable.tar.gz
```
...
powershell -ExecutionPolicy Bypass -File packaging/windows/build-windows.ps1 -Version 2.0.14
# Output: dist/ELI_v2-2.0.14-Setup.exe
```
...
2. Tag: `v2.0.14` (semver matches `pyproject.toml`)
3. Attach:
   - `ELI_v2-2.0.14-linux-portable.tar.gz`
   - `ELI_v2-2.0.14-x86_64.AppImage` (grandma-friendly)
   - `ELI_v2-2.0.14-Setup.exe` (Windows, if built)
   - `.sha256` sidecars
